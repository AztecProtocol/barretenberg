...
if [ "$OS" == "linux" ]; then
  ...
  curl -s -L https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-20.threads/wasi-sdk-20.0+threads-$OS.tar.gz | tar zxfv -
  ...
  curl -s -L curl -O -L https://wasi-sdk.s3.eu-west-2.amazonaws.com/yamt-wasi-sysroot-20.0.threads.tgz | tar zxfv -
fi
tar zxfv wasi-sysroot-20.0+threads.yamt.tgz
cd ..
