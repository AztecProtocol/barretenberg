...
template <typename T> struct allocator {
    ...
    void construct(pointer p, const_reference val) { return alloc_.construct(p, val); }
    ...
    size_type max_size() const noexcept { return std::allocator_traits<decltype(alloc_)>::max_size(alloc_); }
#endif // MSGPACK_CPP_VERSION <= 201703
#endif // defined(MSGPACK_USE_CPP03)
    pointer allocate(size_type n) { return alloc_.allocate(n); }
    void deallocate(pointer p, size_type n) { return alloc_.deallocate(p, n); }

#if MSGPACK_CPP_VERSION <= 201703
    void destroy(pointer p) { alloc_.destroy(p); }
#else  // MSGPACK_CPP_VERSION <= 201703
    void destroy(pointer p) { std::allocator_traits<decltype(alloc_)>::destroy(alloc_, p); }
#endif // MSGPACK_CPP_VERSION <= 201703

    std::allocator<T> alloc_;
};

template <typename T> inline bool operator==(allocator<T> const& lhs, allocator<T> const& rhs)
{
