...
BOOST_AUTO_TEST_CASE(simple_buffer_float)
{
    ...
    for (unsigned int i = 0; i < v.size(); i++) {
        ...
        if (val1 == val1 && val1 >= float(std::numeric_limits<int64_t>::min()) &&
            val1 <= float(std::numeric_limits<int64_t>::max()) && val1 == float(int64_t(val1)))
...
namespace {
template <typename F, typename I> struct TypePair {
...
typedef boost::mpl::list<TypePair<float, signed long long>,
                         ...
                         TypePair<double, unsigned long long>>
...
BOOST_AUTO_TEST_CASE_TEMPLATE(simple_buffer, TypeParam, IntegerToFloatingPointTestTypes)
{
    ...
    v.push_back(0);
    v.push_back(1);
    if (numeric_limits<integer_type>::is_signed)
        v.push_back(static_cast<integer_type>(-1));
    else
        v.push_back(2);
    for (unsigned int i = 0; i < kLoop; i++) {
...
BOOST_AUTO_TEST_CASE(simple_buffer_double)
{
    ...
    for (unsigned int i = 0; i < v.size(); i++) {
        ...
        if (val1 == val1 && val1 >= double(std::numeric_limits<int64_t>::min()) &&
            val1 <= double(std::numeric_limits<int64_t>::max()) && val1 == double(int64_t(val1)))
            BOOST_REQUIRE_EQUAL(sbuf.size(), 1);
        else
...
BOOST_AUTO_TEST_CASE(simple_buffer_fixext1)
{
    ...
    msgpack::object_handle oh2 = msgpack::unpack(sbuf2.data(), sbuf2.size());
...
BOOST_AUTO_TEST_CASE(simple_buffer_fixext2)
{
    ...
    BOOST_CHECK(std::equal(buf, buf + sizeof(buf), oh.get().via.ext.data()));
    ...
    BOOST_CHECK(std::equal(buf, buf + sizeof(buf), oh2.get().via.ext.data()));
...
BOOST_AUTO_TEST_CASE(simple_buffer_ext_convert)
{
    ...
    BOOST_CHECK(std::equal(buf, buf + sizeof(buf), e.data()));
}
...
BOOST_AUTO_TEST_CASE(simple_buffer_ext_pack_convert)
{
    ...
    BOOST_CHECK(std::equal(buf, buf + sizeof(buf), val2.data()));
}
...
BOOST_AUTO_TEST_CASE(simple_buffer_ext_ref_convert)
{
    ...
    BOOST_CHECK(std::equal(buf, buf + sizeof(buf), er.data()));
}
...
BOOST_AUTO_TEST_CASE(simple_buffer_ext_ref_pack_convert)
{
    ...
    BOOST_CHECK_EQUAL(77, val2.type());
    BOOST_CHECK(std::equal(&buf[1], &buf[buf_size], val2.data()));
}
