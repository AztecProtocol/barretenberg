...
BOOST_FUSION_DEFINE_STRUCT(BOOST_PP_EMPTY(), mystruct, (int, f1)(double, f2))
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS)
{
    ...
    template <> struct as<no_def_con1> {
    ...
    template <> struct as<no_def_con2> {
...
struct mystruct_no_def_con {
    ...
    // See *1, *2, and *3
    mystruct_no_def_con(no_def_con1 i, no_def_con2 j, no_def_con1 k)
        : f1(std::move(i))
        , f2(std::move(j))
        , f3(std::move(k))
    {}
...
BOOST_FUSION_ADAPT_STRUCT(mystruct_no_def_con,
                          f1, // *1
                          f2, // *2
                          f3  // *3
)
...
struct mystruct_no_def_con_def_con {
    ...
    // Constructor that have parameters corresponding to BOOST_FUSION_ADAPT_STRUCT is mandatory.
    // See *1, *2, and *3
    mystruct_no_def_con_def_con(no_def_con1 i, no_def_con2 j, int k)
        : f1(std::move(i))
        , f2(std::move(j))
        , f3(std::move(k))
    {}
...
BOOST_FUSION_ADAPT_STRUCT(mystruct_no_def_con_def_con,
                          f1, // *1
                          f2, // *2
                          f3  // *3
)
