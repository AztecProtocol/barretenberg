...
#define MSGPACK_PP_LIST_TRANSFORM(op, data, list)                                                                      \
    MSGPACK_PP_TUPLE_ELEM(                                                                                             \
        3, 2, MSGPACK_PP_LIST_FOLD_RIGHT(MSGPACK_PP_LIST_TRANSFORM_O, (op, data, MSGPACK_PP_NIL), list))
#else
#define MSGPACK_PP_LIST_TRANSFORM(op, data, list) MSGPACK_PP_LIST_TRANSFORM_I(op, data, list)
#define MSGPACK_PP_LIST_TRANSFORM_I(op, data, list)                                                                    \
    MSGPACK_PP_TUPLE_ELEM(                                                                                             \
        3, 2, MSGPACK_PP_LIST_FOLD_RIGHT(MSGPACK_PP_LIST_TRANSFORM_O, (op, data, MSGPACK_PP_NIL), list))
#endif
#
#if ~MSGPACK_PP_CONFIG_FLAGS() & MSGPACK_PP_CONFIG_EDG()
#define MSGPACK_PP_LIST_TRANSFORM_O(d, odr, elem)                                                                      \
    MSGPACK_PP_LIST_TRANSFORM_O_D(                                                                                     \
        d, MSGPACK_PP_TUPLE_ELEM(3, 0, odr), MSGPACK_PP_TUPLE_ELEM(3, 1, odr), MSGPACK_PP_TUPLE_ELEM(3, 2, odr), elem)
#else
...
#
#if ~MSGPACK_PP_CONFIG_FLAGS() & MSGPACK_PP_CONFIG_EDG()
#define MSGPACK_PP_LIST_TRANSFORM_D(d, op, data, list)                                                                 \
    MSGPACK_PP_TUPLE_ELEM(                                                                                             \
        3, 2, MSGPACK_PP_LIST_FOLD_RIGHT_##d(MSGPACK_PP_LIST_TRANSFORM_O, (op, data, MSGPACK_PP_NIL), list))
#else
#define MSGPACK_PP_LIST_TRANSFORM_D(d, op, data, list) MSGPACK_PP_LIST_TRANSFORM_D_I(d, op, data, list)
#define MSGPACK_PP_LIST_TRANSFORM_D_I(d, op, data, list)                                                               \
    MSGPACK_PP_TUPLE_ELEM(                                                                                             \
        3, 2, MSGPACK_PP_LIST_FOLD_RIGHT_##d(MSGPACK_PP_LIST_TRANSFORM_O, (op, data, MSGPACK_PP_NIL), list))
#endif
#
