...
#define MSGPACK_DEFINE_MAP_EACH_PROC(r, data, elem)                                                                    \
    MSGPACK_PP_IF(MSGPACK_PP_IS_BEGIN_PARENS(elem), elem, (MSGPACK_PP_STRINGIZE(elem))(elem))

#define MSGPACK_DEFINE_MAP_IMPL(...)                                                                                   \
    MSGPACK_PP_SEQ_TO_TUPLE(                                                                                           \
        MSGPACK_PP_SEQ_FOR_EACH(MSGPACK_DEFINE_MAP_EACH_PROC, 0, MSGPACK_PP_VARIADIC_TO_SEQ(__VA_ARGS__)))
...
#define MSGPACK_DEFINE_MAP(...)                                                                                        \
    ...
    template <typename MSGPACK_OBJECT> void msgpack_object(MSGPACK_OBJECT* msgpack_o, msgpack::zone& msgpack_z) const  \
    {                                                                                                                  \
...
#define MSGPACK_DEFINE_MAP_EACH_PROC(r, data, elem)                                                                    \
    BOOST_PP_IF(BOOST_PP_IS_BEGIN_PARENS(elem), elem, (BOOST_PP_STRINGIZE(elem))(elem))

#define MSGPACK_DEFINE_MAP_IMPL(...)                                                                                   \
    BOOST_PP_SEQ_TO_TUPLE(BOOST_PP_SEQ_FOR_EACH(MSGPACK_DEFINE_MAP_EACH_PROC, 0, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)))
...
#define MSGPACK_ADD_ENUM(enum_name)                                                                                    \
    ...
    {                                                                                                                  \
        /** @endcond */                                                                                                \
        namespace adaptor {                                                                                            \
        template <> struct convert<enum_name> {                                                                        \
        ...
        template <> struct object<enum_name> {                                                                         \
            void operator()(msgpack::object& msgpack_o, const enum_name& msgpack_v) const                              \
            {                                                                                                          \
                msgpack::underlying_type<enum_name>::type tmp =                                                        \
                    static_cast<msgpack::underlying_type<enum_name>::type>(msgpack_v);                                 \
                msgpack::operator<<(msgpack_o, tmp);                                                                   \
            }                                                                                                          \
        };                                                                                                             \
        template <> struct object_with_zone<enum_name> {                                                               \
