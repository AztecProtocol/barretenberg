...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    class object_handle {
      public:
        ...
        object_handle(msgpack::object const& obj,
            ...
            : m_obj(obj)
            , m_zone(msgpack::move(z))
        {}

        void set(msgpack::object const& obj) { m_obj = obj; }

        /// Get object reference
        /**
         * @return object
         */
        const msgpack::object& get() const { return m_obj; }

        /**
         * @return object (to mimic smart pointers).
         */
        const msgpack::object& operator*() const { return get(); }
         ...
         */
        msgpack::unique_ptr<msgpack::zone>& zone() { return m_zone; }

        /// Get unique_ptr const reference of zone.
        /**
         * @return unique_ptr const reference of zone
         */
        const msgpack::unique_ptr<msgpack::zone>& zone() const { return m_zone; }

#if defined(MSGPACK_USE_CPP03)
        struct object_handle_ref {
            object_handle_ref(object_handle* oh)
                : m_oh(oh)
            {}
            object_handle* m_oh;
        };
        ...
        operator object_handle_ref() { return object_handle_ref(this); }
#endif // defined(MSGPACK_USE_CPP03)
    ...
    template <std::size_t N> inline std::size_t add_ext_type_size(std::size_t size) { return size + 1; }

    template <> inline std::size_t add_ext_type_size<4>(std::size_t size)
    {
    ...
    class object_parser {
      private:
        ...
        struct elem {
            ...
            template <typename Visitor> next_ret next(Visitor& v)
            {
                if (rest == 0) {
                    if (is_map) {
                        if (!v.end_map())
      ...
      public:
        explicit object_parser(msgpack::object const& obj)
            : m_current(&obj)
        {}
        template <typename Visitor> void parse(Visitor& v)
        {
            while (true) {
                ...
                case msgpack::type::BOOLEAN:
                    if (!v.visit_boolean(m_current->via.boolean))
                        return;
                    break;
                case msgpack::type::POSITIVE_INTEGER:
                    if (!v.visit_positive_integer(m_current->via.u64))
                        return;
                    break;
                case msgpack::type::NEGATIVE_INTEGER:
                    if (!v.visit_negative_integer(m_current->via.i64))
                        return;
                    break;
                case msgpack::type::FLOAT32:
                    if (!v.visit_float32(static_cast<float>(m_current->via.f64)))
                        return;
                    break;
                case msgpack::type::FLOAT64:
                    if (!v.visit_float64(m_current->via.f64))
                        return;
                    break;
                case msgpack::type::STR:
                    if (!v.visit_str(m_current->via.str.ptr, m_current->via.str.size))
                        return;
                    break;
                case msgpack::type::BIN:
                    if (!v.visit_bin(m_current->via.bin.ptr, m_current->via.bin.size))
                        return;
                    break;
                case msgpack::type::EXT:
                    msgpack::detail::check_container_size<sizeof(std::size_t)>(m_current->via.ext.size);
                    if (!v.visit_ext(m_current->via.ext.ptr, m_current->via.ext.size + 1))
                        return;
                    break;
                case msgpack::type::ARRAY:
                    if (!v.start_array(m_current->via.array.size))
                        return;
                ...
                case msgpack::type::MAP:
                    if (!v.start_map(m_current->via.map.size))
                        return;
    ...
    struct object_stringize_visitor {
        ...
        bool visit_bin(const char* v, uint32_t size)
        {
            ...
            for (uint32_t i = 0; i < size; ++i) {
                std::ios::fmtflags flags(m_os.flags());
                m_os << std::hex << std::setw(2) << std::setfill('0')
                     << static_cast<int>(static_cast<unsigned char>(v[i]));
                m_os.flags(flags);
            }
            m_os << "\"";
            // </barrenteberg>
    ...
    struct aligned_zone_size_visitor {
        ...
        {
            m_size +=
                msgpack::aligned_size(sizeof(msgpack::object) * num_elements, MSGPACK_ZONE_ALIGNOF(msgpack::object));
            return true;
        }
        bool start_array_item() { return true; }
        bool end_array_item() { return true; }
        ...
        bool start_map(uint32_t num_kv_pairs)
        {
            m_size += msgpack::aligned_size(sizeof(msgpack::object_kv) * num_kv_pairs,
    ...
    template <typename T> inline object::implicit_type::operator T() { return obj.as<T>(); }
    ...
    template <typename T, typename Enabler>
    inline void adaptor::object_with_zone<T, Enabler>::operator()(msgpack::object::with_zone& o, T const& v) const
    {
      ...
      private:
        struct object_with_zone_visitor {
            ...
            bool start_array(uint32_t num_elements)
            {
                ...
                m_ptr->via.array.ptr = static_cast<msgpack::object*>(m_zone.allocate_align(
          ...
          private:
            struct elem {
                ...
                msgpack::object* get_item() { return as.obj; }
                msgpack::object* get_key() { return &as.kv->key; }
                msgpack::object* get_val() { return &as.kv->val; }
                union {
                    msgpack::object* obj;
    ...
    // obsolete
    template <typename Type> class define : public Type {
      public:
        ...
        void msgpack_unpack(object const& o) { msgpack::operator>>(o, static_cast<msgpack_type&>(*this)); }
    };
    ...
    struct object_equal_visitor {
        ...
        bool visit_str(const char* v, uint32_t size)
        {
            if (m_ptr->type != msgpack::type::STR || m_ptr->via.str.size != size ||
        ...
        bool visit_bin(const char* v, uint32_t size)
        {
            if (m_ptr->type != msgpack::type::BIN || m_ptr->via.bin.size != size ||
        ...
        bool visit_ext(const char* v, uint32_t size)
        {
            if (m_ptr->type != msgpack::type::EXT || m_ptr->via.ext.size != size - 1 ||
        ...
        bool start_array(uint32_t num_elements)
        {
            if (m_ptr->type != msgpack::type::ARRAY || m_ptr->via.array.size != num_elements) {
                m_result = false;
                return false;
            }
            m_objs.push_back(elem(m_ptr->via.array.ptr));
        ...
        bool start_map(uint32_t num_kv_pairs)
        {
            if (m_ptr->type != msgpack::type::MAP || m_ptr->via.array.size != num_kv_pairs) {
                m_result = false;
                return false;
            }
            m_objs.push_back(elem(m_ptr->via.map.ptr));
      ...
      private:
        struct elem {
            ...
            msgpack::object const* get_item() { return as.obj; }
            msgpack::object const* get_key() { return &as.kv->key; }
            msgpack::object const* get_val() { return &as.kv->val; }
            union {
    ...
    template <typename T> inline bool operator==(const msgpack::object& x, const T& y)
    {
        try {
    ...
    inline object::implicit_type object::convert() const { return object::implicit_type(*this); }

    template <typename T>
    inline typename msgpack::enable_if<!msgpack::is_array<T>::value && !msgpack::is_pointer<T>::value, T&>::type
    ...
    template <typename T, std::size_t N> inline T(&object::convert(T(&v)[N]) const)[N]
    {
        msgpack::operator>>(*this, v);
        return v;
    }
    ...
    template <typename T>
    inline typename msgpack::enable_if<msgpack::is_pointer<T>::value, T>::type object::convert(T v) const
    {
        convert(*v);
        return v;
    }
    ...
    template <typename T> inline bool object::convert_if_not_nil(T & v) const
    {
        if (is_nil()) {
            return false;
        }
    ...
    template <typename T> inline typename std::enable_if<msgpack::has_as<T>::value, T>::type object::as() const
    {
    ...
    inline object::object() { type = msgpack::type::NIL; }

    template <typename T> inline object::object(const T& v) { *this << v; }
    ...
    template <typename T> inline object::object(const T& v, msgpack::zone& z)
    {
        with_zone oz(z);
    ...
    // obsolete
    template <typename T> inline void convert(T & v, msgpack::object const& o) { o.convert(v); }

    // obsolete
    template <typename Stream, typename T> inline void pack(msgpack::packer<Stream> & o, const T& v) { o.pack(v); }

    // obsolete
    template <typename Stream, typename T> inline void pack_copy(msgpack::packer<Stream> & o, T v) { pack(o, v); }
