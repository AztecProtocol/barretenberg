...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    class unpack_limit {
      public:
        unpack_limit(std::size_t array = 0xffffffff,
    ...
    template <typename T> typename msgpack::enable_if<sizeof(T) == 1>::type load(T& dst, const char* n);

    template <typename T> typename msgpack::enable_if<sizeof(T) == 2>::type load(T& dst, const char* n);

    template <typename T> typename msgpack::enable_if<sizeof(T) == 4>::type load(T& dst, const char* n);

    template <typename T> typename msgpack::enable_if<sizeof(T) == 8>::type load(T& dst, const char* n);
     ...
     *
     */
    object_handle unpack(const char* data,
                         std::size_t len,
                         std::size_t& off,
                         bool& referenced,
     ...
     *
     */
    void unpack(object_handle & result,
                const char* data,
                std::size_t len,
                std::size_t& off,
                bool& referenced,
     ...
     *
     */
    msgpack::object unpack(msgpack::zone & z,
                           const char* data,
                           std::size_t len,
                           std::size_t& off,
                           bool& referenced,
     ...
     * @param limit The size limit information of msgpack::object.
     *
     * This function is obsolete. Use the reference inteface version of unpack functions instead of the pointer
     * interface version.
     */
    void unpack(object_handle * result,
                const char* data,
    ...
    parse_return unpack_imp(const char* data,
                            std::size_t len,
                            std::size_t& off,
