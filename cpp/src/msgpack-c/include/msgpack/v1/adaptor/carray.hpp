...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T, std::size_t N> struct convert<T[N]> {
    ...
    template <std::size_t N> struct convert<unsigned char[N]> {
    ...
    template <typename T, std::size_t N> struct pack<T[N]> {
    ...
    template <std::size_t N> struct pack<char[N]> {
        template <typename Stream>
    ...
    template <std::size_t N> struct pack<const char[N]> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const char (&v)[N]) const
        {
            uint32_t size = checked_get_container_size(N);
    ...
    template <std::size_t N> struct pack<unsigned char[N]> {
        template <typename Stream>
    ...
    template <std::size_t N> struct pack<const unsigned char[N]> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const unsigned char (&v)[N]) const
        {
            unsigned char const* p = v;
    ...
    template <typename T, std::size_t N> struct object_with_zone<T[N]> {
        void operator()(msgpack::object::with_zone& o, const T (&v)[N]) const
        {
            uint32_t size = checked_get_container_size(N);
            o.type = msgpack::type::ARRAY;
            msgpack::object* ptr = static_cast<msgpack::object*>(
    ...
    template <std::size_t N> struct object_with_zone<char[N]> {
    ...
    template <std::size_t N> struct object_with_zone<const char[N]> {
        void operator()(msgpack::object::with_zone& o, const char (&v)[N]) const
        {
            char const* p = v;
            uint32_t size = checked_get_container_size(N);
    ...
    template <std::size_t N> struct object_with_zone<unsigned char[N]> {
    ...
    template <std::size_t N> struct object_with_zone<const unsigned char[N]> {
        void operator()(msgpack::object::with_zone& o, const unsigned char (&v)[N]) const
        {
            uint32_t size = checked_get_container_size(N);
            o.type = msgpack::type::BIN;
    ...
    template <std::size_t N> struct object<char[N]> {
    ...
    template <std::size_t N> struct object<const char[N]> {
        void operator()(msgpack::object& o, const char (&v)[N]) const
        {
            char const* p = v;
            uint32_t size = checked_get_container_size(N);
