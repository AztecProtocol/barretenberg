...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T> struct convert<std::reference_wrapper<T>> {
    ...
    template <typename T> struct pack<std::reference_wrapper<T>> {
    ...
    template <typename T> struct object<std::reference_wrapper<T>> {
    ...
    template <typename T> struct object_with_zone<std::reference_wrapper<T>> {
