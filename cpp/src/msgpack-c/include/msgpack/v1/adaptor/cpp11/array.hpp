...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    inline std::array<T, N1 + N2> concat(std::array<T, N1>&& a1,
                                         std::array<T, N2>&& a2,
                                         msgpack::seq<I1...>,
                                         msgpack::seq<I2...>)
    {
    ...
    template <typename T, std::size_t N> struct as_impl {
    ...
    template <typename T> struct as_impl<T, 1> {
    ...
    template <typename T> struct as_impl<T, 0> {
        static std::array<T, 0> as(msgpack::object const&) { return std::array<T, 0>(); }
    };
    ...
    template <typename T, std::size_t N> struct pack<std::array<T, N>> {
    ...
    template <typename T, std::size_t N> struct object_with_zone<std::array<T, N>> {
