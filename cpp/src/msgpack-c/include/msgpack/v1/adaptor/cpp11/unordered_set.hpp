...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    struct as<std::unordered_set<Key, Hash, Compare, Alloc>,
              typename std::enable_if<msgpack::has_as<Key>::value>::type> {
    ...
    struct convert<std::unordered_set<Key, Hash, Compare, Alloc>> {
        msgpack::object const& operator()(msgpack::object const& o,
                                          std::unordered_set<Key, Hash, Compare, Alloc>& v) const
        {
            if (o.type != msgpack::type::ARRAY) {
                THROW msgpack::type_error();
            }
    ...
    struct pack<std::unordered_set<Key, Hash, Compare, Alloc>> {
        ...
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o,
                                            const std::unordered_set<Key, Hash, Compare, Alloc>& v) const
        {
    ...
    template <typename Key, typename Hash, typename Compare, typename Alloc>
    struct as<std::unordered_multiset<Key, Hash, Compare, Alloc>,
              typename std::enable_if<msgpack::has_as<Key>::value>::type> {
    ...
    struct pack<std::unordered_multiset<Key, Hash, Compare, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o,
                                            const std::unordered_multiset<Key, Hash, Compare, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_array(size);
            for (typename std::unordered_multiset<Key, Hash, Compare, Alloc>::const_iterator it(v.begin()),
                 it_end(v.end());
                 it != it_end;
                 ++it) {
                o.pack(*it);
