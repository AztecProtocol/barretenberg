...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <std::size_t N> struct convert<std::array<unsigned char, N>> {
    ...
    template <> struct convert<std::array<unsigned char, 0>> {
        msgpack::object const& operator()(msgpack::object const& o, std::array<unsigned char, 0>&) const { return o; }
    };

    template <std::size_t N> struct pack<std::array<unsigned char, N>> {
    ...
    template <std::size_t N> struct object<std::array<unsigned char, N>> {
    ...
    template <std::size_t N> struct object_with_zone<std::array<unsigned char, N>> {
