...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T, typename Alloc> struct convert<std::deque<T, Alloc>> {
    ...
    template <typename T, typename Alloc> struct pack<std::deque<T, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::deque<T, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_array(size);
            for (typename std::deque<T, Alloc>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it) {
                o.pack(*it);
            }
            return o;
        }
    ...
    template <typename T, typename Alloc> struct object_with_zone<std::deque<T, Alloc>> {
