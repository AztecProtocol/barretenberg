...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    class ext {
      public:
        ...
        const char* data() const { return &m_data[0] + 1; }
        char* data() { return &m_data[0] + 1; }
    ...
    template <> struct convert<msgpack::type::ext> {
    ...
    template <> struct pack<msgpack::type::ext> {
    ...
    template <> struct object_with_zone<msgpack::type::ext> {
    ...
    class ext_ref {
      public:
        ...
        ext_ref()
            : m_ptr(static_cast<char*>(static_cast<void*>(&m_size)))
            , m_size(0)
        {}
        ...
        const char* data() const { return m_ptr + 1; }

        uint32_t size() const { return m_size; }

        int8_t type() const { return static_cast<int8_t>(m_ptr[0]); }

        std::string str() const { return std::string(m_ptr + 1, m_size); }
        ...
        bool operator!=(const ext_ref& x) const { return !(*this == x); }
    ...
    template <> struct convert<msgpack::type::ext_ref> {
    ...
    template <> struct pack<msgpack::type::ext_ref> {
        template <typename Stream>
    ...
    template <> struct object<msgpack::type::ext_ref> {
    ...
    template <> struct object_with_zone<msgpack::type::ext_ref> {
