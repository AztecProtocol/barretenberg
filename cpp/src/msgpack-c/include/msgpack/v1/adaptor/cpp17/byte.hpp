...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <> struct convert<std::byte> {
    ...
    template <> struct pack<std::byte> {
        template <typename Stream> msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, std::byte v) const
        {
    ...
    template <> struct object<std::byte> {
    ...
    template <> struct object_with_zone<std::byte> {
