...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <std::size_t N> struct convert<std::byte[N]> {
    ...
    template <std::size_t N> struct pack<std::byte[N]> {
    ...
    template <std::size_t N> struct pack<const std::byte[N]> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::byte (&v)[N]) const
        {
            std::byte const* p = v;
    ...
    template <std::size_t N> struct object_with_zone<std::byte[N]> {
    ...
    template <std::size_t N> struct object_with_zone<const std::byte[N]> {
        void operator()(msgpack::object::with_zone& o, const std::byte (&v)[N]) const
        {
            uint32_t size = checked_get_container_size(N);
            o.type = msgpack::type::BIN;
