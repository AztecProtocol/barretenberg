...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename Alloc> struct convert<std::vector<unsigned char, Alloc>> {
...
#pragma GCC diagnostic pop
    ...
    template <typename Alloc> struct pack<std::vector<unsigned char, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o,
                                            const std::vector<unsigned char, Alloc>& v) const
        {
    ...
    template <typename Alloc> struct object<std::vector<unsigned char, Alloc>> {
    ...
    template <typename Alloc> struct object_with_zone<std::vector<unsigned char, Alloc>> {
