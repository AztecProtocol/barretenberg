...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T> struct as<std::complex<T>, typename std::enable_if<msgpack::has_as<T>::value>::type> {
    ...
    template <typename T> struct pack<std::complex<T>> {
    ...
    template <typename T> struct object_with_zone<std::complex<T>> {
