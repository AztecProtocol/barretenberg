...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    struct raw_ref {
        ...
        bool operator!=(const raw_ref& x) const { return !(*this == x); }

        bool operator<(const raw_ref& x) const
        {
            if (size == x.size) {
                return (size == 0 ? false : std::memcmp(ptr, x.ptr, size) < 0);
            } else {
                return size < x.size;
            }
        }
    ...
    template <> struct convert<type::raw_ref> {
    ...
    template <> struct pack<type::raw_ref> {
    ...
    template <> struct object<type::raw_ref> {
    ...
    template <> struct object_with_zone<type::raw_ref> {
