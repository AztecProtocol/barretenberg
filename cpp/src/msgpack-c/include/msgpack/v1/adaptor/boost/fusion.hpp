...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T> struct is_std_pair {
        static bool const value = false;
    };

    template <typename T, typename U> struct is_std_pair<std::pair<T, U>> {
        static bool const value = true;
    };

#if !defined(MSGPACK_USE_CPP03)

    template <typename T> struct is_std_tuple {
        static bool const value = false;
    };

    template <typename... Args> struct is_std_tuple<std::tuple<Args...>> {
        static bool const value = true;
    };

#endif // !defined(MSGPACK_USE_CPP03)

    template <typename T> struct is_seq_no_pair_no_tuple {
        static bool const value = boost::fusion::traits::is_sequence<T>::value && !is_std_pair<T>::value
#if !defined(MSGPACK_USE_CPP03)
    ...
    struct as<T,
              typename msgpack::enable_if<
                  ...
                  boost::mpl::fold<T,
                                   ...
                                   boost::mpl::if_<boost::mpl::or_<boost::mpl::_1, msgpack::has_as<boost::mpl::_2>>,
                                                   boost::mpl::bool_<true>,
        ...
        T operator()(msgpack::object const& o) const
        {
            ...
            return to_t(o.as<tuple_t>(), msgpack::gen_seq<boost::mpl::size<T>::value>());
        }
        template <std::size_t... Is, typename U>
        static std::tuple<typename std::remove_reference<
            typename boost::fusion::result_of::at_c<T, static_cast<int>(Is)>::type>::type...>
        to_tuple(U const& u, seq<Is...>)
        {
            return std::make_tuple(boost::fusion::at_c<Is>(u)...);
        }
        template <std::size_t... Is, typename U> static T to_t(U const& u, seq<Is...>) { return T(std::get<Is>(u)...); }
    };
    ...
    struct convert<T, typename msgpack::enable_if<detail::is_seq_no_pair_no_tuple<T>::value>::type> {
      ...
      private:
        struct convert_imp {
            convert_imp(msgpack::object const& obj, uint32_t& index)
                : obj_(obj)
                , index_(index)
            {}
    ...
    template <typename T> struct pack<T, typename msgpack::enable_if<detail::is_seq_no_pair_no_tuple<T>::value>::type> {
        template <typename Stream> msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const T& v) const
        {
      ...
      private:
        template <typename Stream> struct pack_imp {
            pack_imp(msgpack::packer<Stream>& stream)
                : stream_(stream)
            {}
            template <typename U> void operator()(U const& v) const { stream_.pack(v); }
    ...
    struct object_with_zone<T, typename msgpack::enable_if<detail::is_seq_no_pair_no_tuple<T>::value>::type> {
      ...
      private:
        struct with_zone_imp {
            with_zone_imp(msgpack::object::with_zone const& obj, uint32_t& count)
                : obj_(obj)
                , count_(count)
            {}
