...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <std::size_t N> inline void check_container_size(std::size_t size)
    {
        if (size > 0xffffffff)
            THROW container_size_overflow("container size overflow");
    }

    template <> inline void check_container_size<4>(std::size_t /*size*/) {}

    template <std::size_t N> inline void check_container_size_for_ext(std::size_t size)
    {
        if (size > 0xffffffff)
            THROW container_size_overflow("container size overflow");
    }

    template <> inline void check_container_size_for_ext<4>(std::size_t size)
    {
        if (size > 0xfffffffe)
            THROW container_size_overflow("container size overflow");
    }
    ...
    template <typename T> inline uint32_t checked_get_container_size(T size)
    {
