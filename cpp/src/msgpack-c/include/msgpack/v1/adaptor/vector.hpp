...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename T, typename Alloc> struct convert<std::vector<T, Alloc>> {
    ...
    template <typename T, typename Alloc> struct pack<std::vector<T, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::vector<T, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_array(size);
            for (typename std::vector<T, Alloc>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it) {
                o.pack(*it);
            }
            return o;
        }
    ...
    template <typename T, typename Alloc> struct object_with_zone<std::vector<T, Alloc>> {
