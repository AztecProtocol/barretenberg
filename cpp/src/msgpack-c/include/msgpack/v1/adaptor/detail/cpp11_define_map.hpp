...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename Tuple, std::size_t N> struct define_map_imp {
        template <typename Packer> static void pack(Packer& pk, Tuple const& t)
        {
            define_map_imp<Tuple, N - 1>::pack(pk, t);
            pk.pack(std::get<N - 1>(t));
        }
        static void unpack(msgpack::object const& o,
                           Tuple const& t,
    ...
    template <typename Tuple> struct define_map_imp<Tuple, 0> {
        template <typename Packer> static void pack(Packer&, Tuple const&) {}
    ...
    template <typename... Args> struct define_map {
        ...
        void msgpack_object(msgpack::object* o, msgpack::zone& z) const
        {
            ...
            o->via.map.ptr = static_cast<msgpack::object_kv*>(z.allocate_align(
                sizeof(msgpack::object_kv) * sizeof...(Args) / 2, MSGPACK_ZONE_ALIGNOF(msgpack::object_kv)));
    ...
    template <typename... Args> inline define_map<Args...> make_define_map(Args&... args)
    {
