...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename Tuple> struct define_array_imp<Tuple, 1> {
        template <typename Packer> static void pack(Packer& pk, Tuple const& t) { pk.pack(std::get<0>(t)); }
        static void unpack(msgpack::object const& o, Tuple& t)
        {
            const size_t size = o.via.array.size;
            if (size <= 0) {
    ...
    template <typename... Args> struct define_array {
        ...
        void msgpack_object(msgpack::object* o, msgpack::zone& z) const
        {
            ...
            o->via.array.ptr = static_cast<msgpack::object*>(
                z.allocate_align(sizeof(msgpack::object) * sizeof...(Args), MSGPACK_ZONE_ALIGNOF(msgpack::object)));
