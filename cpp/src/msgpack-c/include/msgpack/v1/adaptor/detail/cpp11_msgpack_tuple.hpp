...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <class... Args> inline tuple<Args...> make_tuple(Args&&... args)
    {
        return tuple<Args...>(std::forward<Args>(args)...);
    }

    template <class... Args> inline tuple<Args&&...> forward_as_tuple(Args&&... args) noexcept
    {
        return tuple<Args&&...>(std::forward<Args>(args)...);
    }
    ...
    inline auto tuple_cat(Tuples&&... args)
        -> decltype(std::tuple_cat(std::forward<typename std::remove_reference<Tuples>::type::base>(args)...))
    {
        return std::tuple_cat(std::forward<typename std::remove_reference<Tuples>::type::base>(args)...);
    }

    template <class... Args> inline tuple<Args&...> tie(Args&... args) { return tuple<Args&...>(args...); }
    } // namespace type

    // --- Pack from tuple to packer stream ---
    template <typename Stream, typename Tuple, std::size_t N> struct MsgpackTuplePacker {
    ...
    template <typename Stream, typename Tuple> struct MsgpackTuplePacker<Stream, Tuple, 1> {
        static void pack(msgpack::packer<Stream>& o, const Tuple& v) { o.pack(v.template get<0>()); }
    };
    ...
    template <typename... Args> struct pack<msgpack::type::tuple<Args...>> {
    ...
    template <typename T, typename... Args> struct MsgpackTupleAsImpl {
    ...
    template <typename... Args> struct MsgpackTupleAs {
        static msgpack::type::tuple<Args...> as(msgpack::object const& o) { return MsgpackTupleAsImpl<Args...>::as(o); }
    };

    template <> struct MsgpackTupleAs<> {
        static msgpack::type::tuple<> as(msgpack::object const&) { return msgpack::type::tuple<>(); }
    };
    ...
    template <typename Tuple, std::size_t N> struct MsgpackTupleConverter {
        static void convert(msgpack::object const& o, Tuple& v)
        {
            ...
            if (o.via.array.size >= N)
                o.via.array.ptr[N - 1].convert<typename std::remove_reference<decltype(v.template get<N - 1>())>::type>(
                    v.template get<N - 1>());
        }
    };
    ...
    template <typename Tuple> struct MsgpackTupleConverter<Tuple, 1> {
        static void convert(msgpack::object const& o, Tuple& v)
        {
            o.via.array.ptr[0].convert<typename std::remove_reference<decltype(v.template get<0>())>::type>(
                v.template get<0>());
        }
    };
    ...
    struct as<msgpack::type::tuple<Args...>,
    ...
    template <typename... Args> struct convert<msgpack::type::tuple<Args...>> {
    ...
    template <typename Tuple, std::size_t N> struct MsgpackTupleToObjectWithZone {
    ...
    template <typename... Args> struct object_with_zone<msgpack::type::tuple<Args...>> {
        void operator()(msgpack::object::with_zone& o, msgpack::type::tuple<Args...> const& v) const
        {
            ...
            o.via.array.ptr = static_cast<msgpack::object*>(o.zone.allocate_align(
                sizeof(msgpack::object) * sizeof...(Args), MSGPACK_ZONE_ALIGNOF(msgpack::object)));
