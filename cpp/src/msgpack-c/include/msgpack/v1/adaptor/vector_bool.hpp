...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    template <typename Alloc> struct convert<std::vector<bool, Alloc>> {
        msgpack::object const& operator()(msgpack::object const& o, std::vector<bool, Alloc>& v) const
        {
            ...
            if (o.via.array.size > 0) {
                ...
                for (typename std::vector<bool, Alloc>::iterator it = v.begin(), end = v.end(); it != end; ++it) {
                    *it = p->as<bool>();
                    ++p;
                }
            }
    ...
    template <typename Alloc> struct pack<std::vector<bool, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::vector<bool, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_array(size);
            for (typename std::vector<bool, Alloc>::const_iterator it(v.begin()), it_end(v.end()); it != it_end; ++it) {
    ...
    template <typename Alloc> struct object_with_zone<std::vector<bool, Alloc>> {
