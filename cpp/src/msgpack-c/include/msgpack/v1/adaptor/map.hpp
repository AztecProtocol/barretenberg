...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    namespace detail {
    template <typename K, typename V, typename Compare, typename Alloc> struct pair_first_less {
    ...
    template <typename K, typename V, typename Compare, typename Alloc>
    struct as<type::assoc_vector<K, V, Compare, Alloc>,
        ...
        void operator()(msgpack::object::with_zone& o, const type::assoc_vector<K, V, Compare, Alloc>& v) const
        {
            ...
            if (v.empty()) {
                ...
                msgpack::object_kv* p = static_cast<msgpack::object_kv*>(
                    o.zone.allocate_align(sizeof(msgpack::object_kv) * size, MSGPACK_ZONE_ALIGNOF(msgpack::object_kv)));
    ...
    template <typename K, typename V, typename Compare, typename Alloc>
    struct as<std::map<K, V, Compare, Alloc>,
              typename std::enable_if<msgpack::has_as<K>::value || msgpack::has_as<V>::value>::type> {
    ...
    template <typename K, typename V, typename Compare, typename Alloc> struct convert<std::map<K, V, Compare, Alloc>> {
    ...
    template <typename K, typename V, typename Compare, typename Alloc> struct pack<std::map<K, V, Compare, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, const std::map<K, V, Compare, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_map(size);
            for (typename std::map<K, V, Compare, Alloc>::const_iterator it(v.begin()), it_end(v.end()); it != it_end;
                 ++it) {
                o.pack(it->first);
                o.pack(it->second);
            }
...
#pragma GCC diagnostic pop
    ...
    template <typename K, typename V, typename Compare, typename Alloc>
    struct as<std::multimap<K, V, Compare, Alloc>,
              typename std::enable_if<msgpack::has_as<K>::value || msgpack::has_as<V>::value>::type> {
    ...
    struct pack<std::multimap<K, V, Compare, Alloc>> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o,
                                            const std::multimap<K, V, Compare, Alloc>& v) const
        {
            uint32_t size = checked_get_container_size(v.size());
            o.pack_map(size);
