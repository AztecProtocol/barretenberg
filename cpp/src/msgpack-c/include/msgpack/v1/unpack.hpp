...
MSGPACK_API_VERSION_NAMESPACE(v1)
{
    ...
    inline void unpack_nil(msgpack::object& o) { o.type = msgpack::type::NIL; }
    ...
    struct unpack_array {
        void operator()(unpack_user& u, uint32_t n, msgpack::object& o) const
        {
            ...
            o.via.array.ptr =
                static_cast<msgpack::object*>(u.zone().allocate_align(size, MSGPACK_ZONE_ALIGNOF(msgpack::object)));
        }
    };
    ...
    struct unpack_map {
        void operator()(unpack_user& u, uint32_t n, msgpack::object& o) const
        {
            ...
            o.via.map.ptr = static_cast<msgpack::object_kv*>(
                u.zone().allocate_align(size, MSGPACK_ZONE_ALIGNOF(msgpack::object_kv)));
        }
    };
    ...
    template <typename T> inline typename msgpack::enable_if<sizeof(T) == 1>::type load(T& dst, const char* n)
    {
        dst = static_cast<T>(*reinterpret_cast<const uint8_t*>(n));
    }

    template <typename T> inline typename msgpack::enable_if<sizeof(T) == 2>::type load(T& dst, const char* n)
    {
        _msgpack_load16(T, n, &dst);
    }

    template <typename T> inline typename msgpack::enable_if<sizeof(T) == 4>::type load(T& dst, const char* n)
    {
        _msgpack_load32(T, n, &dst);
    }

    template <typename T> inline typename msgpack::enable_if<sizeof(T) == 8>::type load(T& dst, const char* n)
    {
    ...
    class context {
      public:
        ...
        msgpack::object const& data() const { return m_stack[0].obj(); }

        unpack_user& user() { return m_user; }

        unpack_user const& user() const { return m_user; }

        int execute(const char* data, std::size_t len, std::size_t& off);

      private:
        template <typename T> static uint32_t next_cs(T p) { return static_cast<uint32_t>(*p) & 0x1f; }

        template <typename T, typename Func>
        int push_aggregate(
            Func const& f, uint32_t container_type, msgpack::object& obj, const char* load_pos, std::size_t& off)
        {
        ...
        template <std::size_t N> static void check_ext_size(std::size_t /*size*/) {}
    ...
    template <> inline void context::check_ext_size<4>(std::size_t size)
    {
        if (size == 0xffffffff)
            THROW msgpack::ext_size_overflow("ext size overflow");
    }
        ...
        do {
            if (m_cs == MSGPACK_CS_HEADER) {
                ...
                if (0x00 <= selector && selector <= 0x7f) { // Positive Fixnum
                    ...
                    int ret = push_aggregate<fix_tag>(unpack_array(), MSGPACK_CT_ARRAY_ITEM, obj, m_current, off);
                    if (ret != 0)
                        return ret;
                } else if (0x80 <= selector && selector <= 0x8f) { // FixMap
                    int ret = push_aggregate<fix_tag>(unpack_map(), MSGPACK_CT_MAP_KEY, obj, m_current, off);
                    if (ret != 0)
                        return ret;
            ...
            if (m_cs != MSGPACK_CS_HEADER || fixed_trail_again) {
                ...
                case MSGPACK_CS_ARRAY_16: {
                    int ret = push_aggregate<uint16_t>(unpack_array(), MSGPACK_CT_ARRAY_ITEM, obj, n, off);
                    if (ret != 0)
                        return ret;
                } break;
                case MSGPACK_CS_ARRAY_32: {
                    /* FIXME security guard */
                    int ret = push_aggregate<uint32_t>(unpack_array(), MSGPACK_CT_ARRAY_ITEM, obj, n, off);
                    if (ret != 0)
                        return ret;
                } break;
                case MSGPACK_CS_MAP_16: {
                    int ret = push_aggregate<uint16_t>(unpack_map(), MSGPACK_CT_MAP_KEY, obj, n, off);
                    if (ret != 0)
                        return ret;
                } break;
                case MSGPACK_CS_MAP_32: {
                    /* FIXME security guard */
                    int ret = push_aggregate<uint32_t>(unpack_map(), MSGPACK_CT_MAP_KEY, obj, n, off);
                    if (ret != 0)
                        return ret;
                } break;
                default:
    ...
    inline unpacker::unpacker(
        unpack_reference_func f, void* user_data, std::size_t initial_buffer_size, unpack_limit const& limit)
        : m_z(new msgpack::zone)
        , m_ctx(f, user_data, limit)
    {
    ...
    {
        if (m_used == m_off && detail::get_count(m_buffer) == 1 && !m_ctx.user().referenced()) {
            // rewind buffer
    ...
    inline char* unpacker::buffer() { return m_buffer + m_used; }

    inline std::size_t unpacker::buffer_capacity() const { return m_free; }
    ...
    inline bool unpacker::next(msgpack::object_handle * result) { return next(*result); }
    ...
    inline msgpack::object const& unpacker::data() { return m_ctx.data(); }
    ...
    inline void unpacker::reset_zone() { m_z->clear(); }
    ...
    inline std::size_t unpacker::message_size() const { return m_parsed - m_off + m_used; }

    inline std::size_t unpacker::parsed_size() const { return m_parsed; }

    inline char* unpacker::nonparsed_buffer() { return m_buffer + m_off; }

    inline std::size_t unpacker::nonparsed_size() const { return m_used - m_off; }

    inline void unpacker::skip_nonparsed_buffer(std::size_t size) { m_off += size; }

    inline void unpacker::remove_nonparsed_buffer() { m_used = m_off; }

    namespace detail {

    inline parse_return unpack_imp(const char* data,
                                   std::size_t len,
                                   std::size_t& off,
    ...
    inline msgpack::object_handle unpack(const char* data,
                                         std::size_t len,
                                         std::size_t& off,
                                         bool& referenced,
                                         ...
                                         unpack_limit const& limit)
    {
        ...
        referenced = false;
        std::size_t noff = off;
        parse_return ret = detail::unpack_imp(data, len, noff, *z, obj, referenced, f, user_data, limit);
    ...
    inline msgpack::object_handle unpack(
        const char* data, std::size_t len, unpack_reference_func f, void* user_data, unpack_limit const& limit)
    {
        bool referenced;
        std::size_t off = 0;
        return unpack(data, len, off, referenced, f, user_data, limit);
    ...
    inline void unpack(msgpack::object_handle & result,
                       const char* data,
                       std::size_t len,
                       std::size_t& off,
                       bool& referenced,
