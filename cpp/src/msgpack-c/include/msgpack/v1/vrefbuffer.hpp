    ...
    class vrefbuffer {
      ...
      public:
            ...
            , m_chunk_size(chunk_size)
        {
            ...
            size_t nfirst = (sizeof(iovec) < 72 / 2) ? 72 / sizeof(iovec) : 8;

            iovec* array = static_cast<iovec*>(::malloc(sizeof(iovec) * nfirst));
            if (!array) {
                THROW std::bad_alloc();
            }
      ...
      public:
        ...
        void append_copy(const char* buf, size_t len)
        {
            ...
            if (m_tail != m_array && m == static_cast<const char*>(const_cast<const void*>((m_tail - 1)->iov_base)) +
                                              (m_tail - 1)->iov_len) {
        ...
        const iovec* vector() const { return m_array; }

        size_t vector_size() const { return static_cast<size_t>(m_tail - m_array); }
