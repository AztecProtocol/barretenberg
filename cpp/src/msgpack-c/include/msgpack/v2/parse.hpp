...
MSGPACK_API_VERSION_NAMESPACE(v2)
{
    ...
    template <typename VisitorHolder> class context {
      public:
        context()
            : m_trail(0)
      ...
      private:
        ...
        VisitorHolder& holder() { return static_cast<VisitorHolder&>(*this); }

        template <typename T, typename StartVisitor, typename EndVisitor>
        parse_return start_aggregate(StartVisitor const& sv,
                                     EndVisitor const& ev,
                                     const char* load_pos,
                                     std::size_t& off)
        {
        ...
        struct array_sv {
            ...
            {}
            bool operator()(uint32_t size) const { return m_visitor_holder.visitor().start_array(size); }
        ...
        struct array_ev {
            array_ev(VisitorHolder& visitor_holder)
                : m_visitor_holder(visitor_holder)
            {}
            bool operator()() const { return m_visitor_holder.visitor().end_array(); }
        ...
        struct map_sv {
            map_sv(VisitorHolder& visitor_holder)
                : m_visitor_holder(visitor_holder)
            {}
            bool operator()(uint32_t size) const { return m_visitor_holder.visitor().start_map(size); }
        ...
        struct map_ev {
            map_ev(VisitorHolder& visitor_holder)
                : m_visitor_holder(visitor_holder)
            {}
            bool operator()() const { return m_visitor_holder.visitor().end_map(); }
        ...
        struct unpack_stack {
            ...
            unpack_stack() { m_stack.reserve(MSGPACK_EMBED_STACK_SIZE); }
    ...
    template <std::size_t N> inline void check_ext_size(std::size_t /*size*/) {}

    template <> inline void check_ext_size<4>(std::size_t size)
    {
        if (size == 0xffffffff)
    ...
    inline parse_return context<VisitorHolder>::execute(const char* data, std::size_t len, std::size_t& off)
    {
        ...
        do {
            ...
            if (m_cs != MSGPACK_CS_HEADER || fixed_trail_again) {
                ...
                default:
                    off = static_cast<std::size_t>(m_current - m_start);
                    holder().visitor().parse_error(static_cast<std::size_t>(n - m_start - 1),
                                                   static_cast<std::size_t>(n - m_start));
                    return PARSE_PARSE_ERROR;
                }
            }
        } while (m_current != pe);
    ...
    template <typename VisitorHolder, typename ReferencedBufferHook>
    inline parser<VisitorHolder, ReferencedBufferHook>::parser(ReferencedBufferHook & hook,
    ...
    template <typename VisitorHolder, typename ReferencedBufferHook>
    inline parser<VisitorHolder, ReferencedBufferHook>& parser<VisitorHolder, ReferencedBufferHook>::operator=(
        this_type&& other)
    {
        this->~parser();
    ...
    template <typename Visitor> inline bool parse(const char* data, size_t len, size_t& off, Visitor& v)
    {
        parse_return ret = msgpack::detail::parse_imp(data, len, off, v);
        return ret == PARSE_SUCCESS || ret == PARSE_EXTRA_BYTES;
    }

    template <typename Visitor> inline bool parse(const char* data, size_t len, Visitor& v)
    {
        std::size_t off = 0;
    ...
    template <typename Visitor> struct parse_helper : detail::context<parse_helper<Visitor>> {
    ...
    template <typename Visitor> inline parse_return parse_imp(const char* data, size_t len, size_t& off, Visitor& v)
    {
        std::size_t noff = off;
