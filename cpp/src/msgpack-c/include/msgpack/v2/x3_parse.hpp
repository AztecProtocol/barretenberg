...
MSGPACK_API_VERSION_NAMESPACE(v2)
{
    ...
    struct index_size {
        enum struct type_t { array, map, other };
        index_size(std::size_t size, type_t type = type_t::other)
            : size(size)
            , type(type)
        {}
        std::size_t index = 0;
    ...
    template <typename Visitor> struct app_specific {
        template <typename Vis>
        app_specific(Vis&& vis)
            : vis(vis)
        {}
    ...
    template <typename Visitor> app_specific<Visitor> make_app_specific(Visitor&& vis)
    {
    ...
    const auto mp_object_def =
        ...
        // -----------------------------------------------
        mp_float64 >> mp_d_uint64[([](auto& ctx) {
            auto& app_specific = x3::get<tag_app_specific>(ctx).get();
            union {
                uint64_t i;
                double f;
        ...
        mp_fixstr[([](auto& ctx) {
            ...
            x3::raw[*(x3::eps[more] >> x3::char_) >> x3::eps[done]][([](auto& ctx) {
                auto& app_specific = x3::get<tag_app_specific>(ctx).get();
        ...
        mp_fixarray[([](auto& ctx) {
            ...
            *(x3::eps[more] >> array_item) >> x3::eps[done] |
        // -----------------------------------------------
        mp_array16 >> mp_d_uint16[([](auto& ctx) {
            auto& app_specific = x3::get<tag_app_specific>(ctx).get();
            ...
            *(x3::eps[more] >> array_item) >> x3::eps[done] |
        // -----------------------------------------------
        mp_array32 >> mp_d_uint32[([](auto& ctx) {
            auto& app_specific = x3::get<tag_app_specific>(ctx).get();
            uint32_t size = _attr(ctx);
            app_specific.index_sizes.emplace_back(size, index_size::type_t::array);
            app_specific.vis.start_array(size);
        ...
        mp_fixmap[([](auto& ctx) {
            ...
            *(x3::eps[more] >> map_item) >> x3::eps[done] |
        // -----------------------------------------------
        mp_map16 >> mp_d_uint16[([](auto& ctx) {
            auto& app_specific = x3::get<tag_app_specific>(ctx).get();
            uint32_t size = _attr(ctx);
            app_specific.index_sizes.emplace_back(size, index_size::type_t::map);
            app_specific.vis.start_map(size);
    ...
    const auto array_item_def = x3::eps[([](auto& ctx) {
                                    auto& app_specific = x3::get<tag_app_specific>(ctx).get();
                                    app_specific.vis.start_array_item();

                                    _pass(ctx) = true;
                                })] >>
                                mp_object >> x3::eps[([](auto& ctx) {
                                    auto& app_specific = x3::get<tag_app_specific>(ctx).get();
    ...
    const auto kv_def = x3::eps[([](auto& ctx) {
                            auto& app_specific = x3::get<tag_app_specific>(ctx).get();
    ...
    BOOST_SPIRIT_DEFINE(mp_object, array_item, map_item, kv);

    const auto rule = mp_object;

    } // namespace detail

    template <typename Iterator, typename Visitor> inline bool parse(Iterator && begin, Iterator && end, Visitor && vis)
    {
        auto data = detail::make_app_specific(std::forward<Visitor>(vis));
        return detail::x3::parse(std::forward<Iterator>(begin),
