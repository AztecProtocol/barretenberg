...
struct json_like_visitor : msgpack::v2::null_visitor {
    ...
    void parse_error(size_t /*parsed_offset*/, size_t /*error_offset*/) { std::cerr << "parse error" << std::endl; }
...
class json_like_printer : public msgpack::parser<json_like_printer, do_nothing>, public json_like_visitor {
...
template <typename T> struct ref_buffer {
    ref_buffer(T& t)
        : t(t)
    {}
...
void consume(const char* buff, const std::size_t size, ref_buffer<json_like_printer>& rb, json_like_printer& jp)
{
    rb.write(buff, size);
    while (jp.next()) {
