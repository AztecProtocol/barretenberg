...
class my_class {
  public:
    ...
    my_class(std::string const& name, int age)
        : name_(name)
        , age_(age)
    {}
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS)
{
    ...
    template <> struct convert<my_class> {
        msgpack::object const& operator()(msgpack::object const& o, my_class& v) const
        {
            ...
            if (o.via.array.size != 2)
                throw msgpack::type_error();
            v = my_class(o.via.array.ptr[0].as<std::string>(), o.via.array.ptr[1].as<int>());
            return o;
        }
    };

    template <> struct pack<my_class> {
        template <typename Stream> packer<Stream>& operator()(msgpack::packer<Stream>& o, my_class const& v) const
        {
    ...
    template <> struct object_with_zone<my_class> {
