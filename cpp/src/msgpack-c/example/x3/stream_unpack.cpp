...
class buffered_iterator : public std::iterator<std::input_iterator_tag, char> {
  public:
    ...
    {
        if (!other.source_ && !source_ && !other.buf_ && !buf_)
            return true;
        return other.it_ == it_;
    }

    bool operator!=(buffered_iterator const& other) const noexcept { return !(other == *this); }
    ...
    reference_t operator*() noexcept { return *it_; }

    pointer_t operator->() noexcept { return std::addressof(*it_); }
...
class server {
  public:
    server(as::io_service& ios, std::uint16_t port)
