...
TYPED_TEST(join_split, deposit)
{
    ...
    // WORKTODO: whoops, using global alias!
...
//         }; value::value_note input_note2 = { 0, 0, 0, user.owner.public_key, user.note_secret, 0,
//         fr::random_element() }; auto input_nullifier1 = compute_nullifier(input_note1.commit(),
//         user.owner.private_key, false); auto input_nullifier2 = compute_nullifier(input_note2.commit(),
//         user.owner.private_key, false); value::value_note output_note1 = { 0, 0, 0, user.owner.public_key,
//         user.note_secret, 0, input_nullifier1 }; value::value_note output_note2 = { 0, 0, 0, user.owner.public_key,
//         user.note_secret, 0, input_nullifier2 };
...
//     bridge_call_data bridge_call_data = empty_bridge_call_data;
//     bridge_call_data.input_asset_id_a = tx.input_note[0].asset_id;
//     bridge_call_data.input_asset_id_b = tx.input_note[1].asset_id + 1; // <-- wrong asset_id, vs input note 2's
//     asset_id bridge_call_data.config.second_input_in_use = true; tx.partial_claim_note.bridge_call_data =
//     bridge_call_data.to_uint256_t();
...
//     auto result = sign_and_verify_logic(tx, user.owner);
//     EXPECT_FALSE(result.valid);
//     // In this context, failure of this subtraction (due to underflow) implies input note 1's value was less than
//     input
