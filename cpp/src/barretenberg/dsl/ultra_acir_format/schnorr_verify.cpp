...
crypto::schnorr::signature convert_signature(plonk::UltraComposer& composer, std::vector<uint32_t> signature)
{
...
byte_array_ct vector_of_bytes_to_byte_array(plonk::UltraComposer& composer, std::vector<uint32_t> vector_of_bytes)
{
...
witness_ct index_to_witness(plonk::UltraComposer& composer, uint32_t index)
{
    fr value = composer.get_variable(index);
    return { &composer, value };
}

void create_schnorr_verify_constraints(plonk::UltraComposer& composer, const SchnorrConstraint& input)
{
