...
struct ultra_acir_format {
    ...
    friend bool operator==(ultra_acir_format const& lhs, ultra_acir_format const& rhs) = default;
};

void read_witness(UltraComposer& composer, std::vector<barretenberg::fr> witness);

void create_circuit(UltraComposer& composer, const ultra_acir_format& constraint_system);

UltraComposer create_circuit(const ultra_acir_format& constraint_system,
                             std::unique_ptr<bonk::ReferenceStringFactory>&& crs_factory);

UltraComposer create_circuit_with_witness(const ultra_acir_format& constraint_system,
                                          std::vector<fr> witness,
                                          std::unique_ptr<ReferenceStringFactory>&& crs_factory);

UltraComposer create_circuit_with_witness(const ultra_acir_format& constraint_system, std::vector<fr> witness);

void create_circuit_with_witness(UltraComposer& composer,
                                 const ultra_acir_format& constraint_system,
                                 std::vector<fr> witness);

// Serialisation
template <typename B> inline void read(B& buf, ultra_acir_format& data)
{
...
template <typename B> inline void write(B& buf, ultra_acir_format const& data)
{
