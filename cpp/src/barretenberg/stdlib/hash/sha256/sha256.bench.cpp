...
using LocalComposer = UltraComposer;
using LocalProver = UltraProver;
using LocalVerifier = UltraVerifier;
...
void generate_test_plonk_circuit(LocalComposer& composer, size_t num_bytes)
{
    std::string in;
    in.resize(num_bytes);
    ...
    stdlib::packed_byte_array<LocalComposer> input(&composer, in);
    stdlib::sha256<LocalComposer>(input);
}

LocalComposer composers[NUM_HASHES];
LocalProver provers[NUM_HASHES];
LocalVerifier verifiers[NUM_HASHES];
