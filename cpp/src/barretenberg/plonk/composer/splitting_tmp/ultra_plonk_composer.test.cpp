...
//     const auto input_hi_index_new = composer_new.add_variable(input_hi);
//     const auto input_lo_index_new = composer_new.add_variable(input_lo);

//     const auto sequence_data_hi_new = plookup::get_lookup_accumulators(MultiTableId::PEDERSEN_LEFT_HI, input_hi);
//     const auto sequence_data_lo_new = plookup::get_lookup_accumulators(MultiTableId::PEDERSEN_LEFT_LO, input_lo);

//     const auto lookup_witnesses_hi_new = composer_new.create_gates_from_plookup_accumulators(
//         MultiTableId::PEDERSEN_LEFT_HI, sequence_data_hi_new, input_hi_index_new);
//     const auto lookup_witnesses_lo_new = composer_new.create_gates_from_plookup_accumulators(
//         MultiTableId::PEDERSEN_LEFT_LO, sequence_data_lo_new, input_lo_index_new);
...
//     // for (size_t i = 0; i < num_lookups_hi; ++i) {
//     //     EXPECT_EQ(composer.get_variable(lookup_witnesses_hi[ColumnIdx::C1][i]), expected_scalars[i +
//     num_lookups_lo]);
//     //     EXPECT_EQ(composer.get_variable(lookup_witnesses_hi[ColumnIdx::C2][i]), expected_x[i + num_lookups_lo]);
//     //     EXPECT_EQ(composer.get_variable(lookup_witnesses_hi[ColumnIdx::C3][i]), expected_y[i + num_lookups_lo]);
//     // }
...
//     info("prover.key->circuit_size = ", prover.key->circuit_size);
//     info("prover_new.key->circuit_size = ", prover_new.key->circuit_size);

//     std::string key = "w_1_lagrange";
//     info(prover.key->polynomial_store.get(key).size());
//     info(prover_new.key->polynomial_store.get(key).size());

//     // for (size_t i = 8000; i< 8189; ++i)
//     // {
//     //     auto val = prover.key->polynomial_store.get(key)[i];
//     //     auto val_new = prover_new.key->polynomial_store.get(key)[i];
//     //     if (val == val_new) { info("Equal: ", i, ", ", val); }
//     //     if (val != val_new) { info("UNEQUAL: ", i, ", ", val); }
//     // }
...
//     // for (const auto& [key, poly] : verifier.key->commitments) {
//     //     // info(key);
//     //     // if(verifier.key->commitments[key] == verifier_new.key->commitments[key]){
//     //     //     info("Equal: ", key);
//     //     // };
//     //     // if(verifier.key->commitments[key] != verifier_new.key->commitments[key]){
//     //     //     info("UNEQUAL: ", key);
...
//     bool result_new = verifier_new.verify_proof(proof_new);
//     (void)result_new;
//     EXPECT_EQ(result_new, true);
