 ...
 *
 * @warning The proof of PlonK's correctness uses the fact that verifier challenges are generated by a hash function (a
 * custom Pedersen hash that uses lookup tables) standing in for a random oracle. UltraPlonK currently uses a hash
 * function that is known to violate this assumption. We plan to switch to an algebraic hash function that is believed
 * to have the random oracle property in a future upgrade.
 */
