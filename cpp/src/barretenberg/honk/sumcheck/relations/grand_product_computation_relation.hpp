...
template <typename FF> class GrandProductComputationRelation {
  public:
     ...
     *
     *  C(extended_edges(X)...) = ( z_perm(X) + lagrange_first(X) )*P(X) - ( z_perm_shift(X) + delta * lagrange_last(X)
     * )*Q(X), where P(X) = Prod_{i=1:3} w_i(X) + β*(n*(i-1) + idx(X)) + γ Q(X) = Prod_{i=1:3} w_i(X) + β*σ_i(X) + γ
     *
