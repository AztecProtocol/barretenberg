  ...
  public:
    ...
    struct MSMState {
        ...
        struct AddState {
            bool add = false;
            int slice = 0;
            grumpkin::g1::affine_element point{ 0, 0 };
            barretenberg::fr lambda = 0;
            barretenberg::fr collision_inverse = 0;
        };
    ...
    static std::vector<MSMState> compute_msm_state(const std::vector<eccvm::MSM>& msms,
                                                   ...
                                                   const uint32_t total_number_of_muls)
    {
        ...
        constexpr size_t num_point_table_read_columns = 2;
        const size_t table_rows = total_number_of_muls * 8;
        ...
        const auto update_read_counts = [&](const size_t pc, const int slice) {
            ...
            bool slice_negative = slice < 0 ? true : false;
        ...
        for (const auto& msm : msms) {
            ...
            for (size_t j = 0; j < num_rounds; ++j) {
                for (size_t k = 0; k < rows_per_round; ++k) {
                    ...
                    // row.accumulator = accumulator.is_point_at_infinity() ? grumpkin::g1::affine_element{ 0, 0 }
                    //                                                      : grumpkin::g1accumulator;
                    row.pc = pc;
                    accumulator = acc;
                ...
                if (j < num_rounds - 1) {
                    ...
                    accumulator = grumpkin::g1::element(accumulator).dbl().dbl().dbl().dbl();
                    // ASSERT(row.accumulator_x == dx);
                    // ASSERT(row.accumulator_y == dy);
                    row.pc = pc;
                    msm_state.push_back(row);
                } else {
                    for (size_t k = 0; k < rows_per_round; ++k) {
            ...
            for (size_t i = 0; i < msm.size(); ++i) {
                expected += (grumpkin::g1::element(msm[i].base_point) * msm[i].scalar);
            }
            ASSERT(accumulator == grumpkin::g1::affine_element(expected));
        }
        ...
        final_row.add_state = { MSMState::AddState{ false, 0, { 0, 0 }, 0, 0 },
                                MSMState::AddState{ false, 0, { 0, 0 }, 0, 0 },
                                MSMState::AddState{ false, 0, { 0, 0 }, 0, 0 },
                                MSMState::AddState{ false, 0, { 0, 0 }, 0, 0 } };
