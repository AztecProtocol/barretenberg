...
namespace test_ultra_honk_relations {

/**
 * @brief Check correctness of Ultra Honk relations on a real circuit
 *
 */
TEST(UltraHonkRelations, SumcheckRelationCorrectness)
{
...
//     // Compute grand product polynomial
//     auto z_perm_poly =
//         prover_library::compute_permutation_grand_product<num_wires>(prover.key, prover.wire_polynomials, beta,
//         gamma);
...
//         // For each relation, call the `accumulate_relation_evaluation` over all witness/selector values at the
//         // i-th row/vertex of the hypercube.
//         // We use ASSERT_EQ instead of EXPECT_EQ so that the tests stops at the first index at which the result is
//         not
//         // 0, since result = 0 + C(transposed), which we expect will equal 0.
//         std::get<0>(relations).add_full_relation_value_contribution(result, evaluations_at_index_i, params);
//         ASSERT_EQ(result, 0);
