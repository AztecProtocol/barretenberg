                                                                 ...
                                                                 const FF& scaling_factor) const
{
    ...
    const auto& Tx = View(extended_edges.table_tx);
    const auto& Tx_shift = View(extended_edges.table_tx_shift);
    const auto& Ty = View(extended_edges.table_ty);
    const auto& Ty_shift = View(extended_edges.table_ty_shift);
    const auto& Dx = View(extended_edges.table_dx);
    const auto& Dx_shift = View(extended_edges.table_dx_shift);
    const auto& Dy = View(extended_edges.table_dy);
    const auto& Dy_shift = View(extended_edges.table_dy_shift);
    const auto& table_point_transition = View(extended_edges.table_point_transition);
     ...
     *
     * | table_pc | table_point_transition  | table_round | Tx    | Ty    | Dx   | Dy   |
     * | -------- | ----------------------- | ----------- | ----- | ----- | ---- | ---- |
     ...
     *
     * 1. If table_point_transition = 0, (Dx, Dy) = (Dx_shift, Dy_shift)
     * 2. If table_point_transition = 1, (Dx, Dy) = 2 (Px, Py)
     * 3. If table_point_transition = 0, (Tx, Ty) = (Tx_shift, Ty_shift) + (Dx, Dy)
     *
     * The relations that constrain `table_point_transition` and `table_pc` are in `ecc_wnaf_relation.hpp`
     *
     * When table_point_transition = 1, we use a strict lookup protocol in `ecc_set_relation.hpp` to validate (pc, Tx,
     ...
     * table:
     * 1. (pc, 15 - table_round, Tx, Ty)
     * 2. (pc, table_round, Tx, -Ty)
     *
     * The value `15 - table_round` describes the multiplier applied to P at the current row.
     * (this can be expanded into a wnaf value by taking `2x - 15` where `x = 15 - table_round`) .
     * The value `table_round` describes the *negative multiplier* applied to P at the current row.
     * This is also expanded into a wnaf value by taking `2x - 15` where `x = table_round`.
     *
     * The following table describes how taking (15 - table_round) for positive values and (table_round) for negative
     * values produces the WNAF slice values that correspond to the multipliers for (Tx, Ty) and (Tx, -Ty):
     *
     * | Tx    | Ty    | x = 15 - table_round | 2x - 15 | y = table_round | 2y - 15 |
     * | ----- | ----- | -------------------- | ------- | --------------- | ------- |
     ...
     * i.e. (Dx, Dy) = 2(Px, Py) when table_round_transition = 1.
     *
     * Double formula:
     * x_3 = 9x^4 / 4y^2 - 2x
     * y_3 = (3x^2 / 2y) * (x - x_3) - y
    ...
    std::get<0>(accumulator) += table_point_transition * x_double_check * scaling_factor;
    std::get<1>(accumulator) += table_point_transition * y_double_check * scaling_factor;

    /**
     * @brief If table_round_transition = 0, (Dx_shift, Dy_shift) = (Dx, Dy)
     *
     * 1st row is empty => don't apply if lagrange_first == 1
     */
    std::get<2>(accumulator) +=
        (-lagrange_first + 1) * (-table_point_transition + 1) * (Dx - Dx_shift) * scaling_factor;
    std::get<3>(accumulator) +=
        (-lagrange_first + 1) * (-table_point_transition + 1) * (Dy - Dy_shift) * scaling_factor;

    /**
     * @brief Valdiate (Tx, Ty) is correctly computed from (Tx_shift, Ty_shift), (Dx, Dy).
     *        If table_round_transition = 0, [T] = [T_shift] + [D].
     *
     * Add formula:
    ...
    std::get<4>(accumulator) += (-lagrange_first + 1) * (-table_point_transition + 1) * x_add_check * scaling_factor;
    std::get<5>(accumulator) += (-lagrange_first + 1) * (-table_point_transition + 1) * y_add_check * scaling_factor;
}
