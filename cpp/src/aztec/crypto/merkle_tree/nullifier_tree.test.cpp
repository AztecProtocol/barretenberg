...
TEST(crypto_nullifier_tree, test_kv_memory_vs_memory_consistency)
{
...
TEST(crypto_nullifier_tree, test_size)
{
    MemoryStore store;
...
TEST(crypto_nullifier_tree, test_get_hash_path)
{
...
TEST(crypto_nullifier_tree, test_leveldb_get_hash_path_layers)
{
    {
        MemoryStore store;
...
#ifndef __wasm__
std::string DB_PATH_ = format("/tmp/leveldb_test_", random_engine.get_random_uint128());

// TODO: Add more tests here
TEST(crypto_nullifier_tree, test_leveldb_vs_memory_consistency)
{
    constexpr size_t depth = 10;
    NullifierMemoryTree memdb(depth);

    LevelDbStore::destroy(DB_PATH_);
    LevelDbStore store(DB_PATH_);
    LevelDbNullifierTree db(store, depth);
