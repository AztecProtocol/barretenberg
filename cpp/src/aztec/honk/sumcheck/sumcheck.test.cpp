...
TEST(Sumcheck, PolynomialNormalization)
{
    ...
    auto sumcheck =
        Sumcheck<FF, ArithmeticRelation, GrandProductComputationRelation, GrandProductInitializationRelation>(
            multivariate_n, {});
    auto prover_transcript = ProverTranscript<FF>{};
    prover_transcript.send_to_verifier("init", FF{ 2 });

    auto [evaluation_points, evals] = sumcheck.execute_prover(full_polynomials, prover_transcript);
...
TEST(Sumcheck, ProverAndVerifier)
{
    ...
    auto prover_output = sumcheck.execute_prover(full_polynomials, prover_transcript);
    ...
    auto verifier_output = sumcheck_verifier.execute_verifier(verifier_transcript);
