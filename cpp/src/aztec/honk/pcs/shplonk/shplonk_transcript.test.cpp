...
template <class Params> class ShplonkTranscriptTest : public CommitmentTest<Params> {
...
TYPED_TEST(ShplonkTranscriptTest, Gemini)
{
    ...
    Gemini::reduce_verify_with_transcript(u, claims, {}, gemini_proof, transcript);

    const auto [prover_claim, witness, proof] =
        Shplonk::reduce_prove_with_transcript(this->ck(), gemini_claim, gemini_witness, transcript);

    this->verify_opening_claim(prover_claim, witness);

    const auto verifier_claim = Shplonk::reduce_verify_with_transcript(gemini_claim, proof, transcript);
    EXPECT_EQ(prover_claim, verifier_claim);
}
} // namespace honk::pcs::shplonk