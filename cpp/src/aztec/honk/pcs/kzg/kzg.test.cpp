...
TYPED_TEST(BilinearAccumulationTest, single)
{
    ...
    auto opening_claim = OpeningClaim<TypeParam>{ .opening_pair = opening_pair, .commitment = commitment };
...
TYPED_TEST(BilinearAccumulationTest, GeminiShplonkKzgWithShift)
{
    ...
    const auto [gemini_prover_claim, gemini_witness] = Gemini::reduce_prove(this->ck(),
                                                                            mle_opening_point,
    ...
    const auto [shplonk_prover_claim, shplonk_witness] =
        Shplonk::reduce_prove(this->ck(), gemini_prover_claim, gemini_witness, prover_transcript);

    // KZG prover:
    // - Adds commitment [W] to transcript
    KZG::reduce_prove(this->ck(), shplonk_prover_claim, shplonk_witness, prover_transcript);
    ...
    const auto gemini_verifier_claim = Gemini::reduce_verify(mle_opening_point,
                                                             batched_evaluation,
    ...
    EXPECT_TRUE(kzg_claim.verify(this->vk()));
}
} // namespace honk::pcs::kzg