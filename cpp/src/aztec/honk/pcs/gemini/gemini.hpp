...
template <typename Params> class MultilinearReductionScheme {
  ...
  public:
    ...
    //     const size_t num_shifted_polys = bonk::StandardArithmetization::NUM_SHIFTED_POLYNOMIALS;
    //     const size_t num_polys = mle_witness_polynomials.size();
    //     ASSERT(claims_f.size() == num_unshifted_polys);
    //     ASSERT(claims_g.size() == num_shifted_polys);
    ...
    //     std::span rhos_f = rhos_span.subspan(0, num_unshifted_polys);
    //     std::span rhos_g = rhos_span.subspan(num_unshifted_polys, num_shifted_polys);
    ...
    //     for (size_t j = 0; j < num_unshifted_polys; ++j) {
    //         // F(X) += ρʲ fⱼ(X)
    //         batched_F.add_scaled(*mle_witness_polynomials[j], rhos[j]);
    //     }

    //     //  G(X) = ∑ⱼ ρʲ gⱼ(X)
    //     Polynomial& batched_G = witness_polynomials.emplace_back(Polynomial(n));
    //     // for (size_t j = 0; j < num_shifted_polys; ++j) {
    //     //     // G(X) += ρʲ gⱼ(X)/X
    //     //     batched_G.add_scaled_unshifted(*mle_witness_polynomials[j], rhos[j]);
    //     // }
    //     batched_G.add_scaled_unshifted(*mle_witness_polynomials[honk::StandardArithmetization::POLYNOMIAL::Z_PERM_SHIFT],
