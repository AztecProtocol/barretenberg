...
template <typename Composer> class stdlib_field : public plonk::stdlib::test::StdlibTest<Composer> {
...
TYPED_TEST_SUITE(stdlib_field, plonk::stdlib::test::ComposerTypes);

TYPED_STDLIB_TEST(stdlib_field, create_range_constraint)
{
    // first run all successful tests
    // we reuse the same composer for all successful test
    // so that we only create one proof and verify them all at once
    {
        auto run_success_test = [&](Composer& composer, fr elt, size_t num_bits) {
            field_ct a(witness_ct(&composer, elt));
            a.create_range_constraint(num_bits, "field_tests: range_constraint on a fails");
            EXPECT_FALSE(composer.failed()) << "Range constraint expected no failure on " << elt << " with num_bits "
                                            << num_bits << "\n\t error: " << composer.err();
        };

        Composer composer;
        run_success_test(composer, 0, 0);
        run_success_test(composer, 0, 1);
        run_success_test(composer, 1, 1);
        run_success_test(composer, 2, 2);
        run_success_test(composer, 3, 2);
        run_success_test(composer, -1, fr::modulus.get_msb() + 1);

        // 130 = 0b10000010, 8 bits, so 8 bits or more should succeed
        for (size_t num_bits = 8; num_bits < 17; num_bits++) {
            run_success_test(composer, 130, num_bits);
        }
    ...
    {
        auto run_failure_test = [&](fr elt, size_t num_bits) {
            Composer composer;
            ...
            EXPECT_TRUE(this->circuit_fails(composer, false))
                << "Range constraint expected failure on " << elt << " with num_bits " << num_bits;
        };

        // 130 = 0b10000010, 8 bits, so anything less should fail
        // for (size_t num_bits = 1; num_bits < 7; num_bits++) {
        for (size_t num_bits = 1; num_bits < 8; num_bits++) {
            run_failure_test(130, num_bits);
        }
        run_failure_test(1, 0);
        run_failure_test(2, 1);
        // -1 has maximum bit length
        run_failure_test(-1, fr::modulus.get_msb());
        run_failure_test(-1, 128);
    }
}
 ...
 */
TYPED_STDLIB_TEST(stdlib_field, assert_equal)
{
    auto run_test = [&](bool constrain, bool true_when_y_val_zero = true) {
        ...
        if (expect_verifed) {
            EXPECT_TRUE(this->circuit_verifies(composer, false));
        } else {
            EXPECT_TRUE(this->circuit_fails(composer));
        }
    };

    run_test(false); // test fails now
    run_test(true, true);
    run_test(true, false); // test also fails now
}

TYPED_STDLIB_TEST(stdlib_field, add_mul_with_constants)
{
    Composer composer;

    uint64_t expected = TestFixture::fidget(composer);
...
TYPED_STDLIB_TEST(stdlib_field, div)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, field_fibbonaci)
{
    Composer composer;

    this->fibbonaci(composer);
...
TYPED_STDLIB_TEST(stdlib_field, field_pythagorean)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, equality)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, equality_false)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, equality_with_constants)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, larger_circuit)
{
    size_t n = 16384;
    Composer composer = Composer("../srs_db/ignition", n);

    this->generate_test_plonk_circuit(composer, n);
...
TYPED_STDLIB_TEST(stdlib_field, is_zero)
{
    Composer composer;
    ...
    field_ct c_1(&composer, uint256_t(0x1122334455667788, 0x8877665544332211, 0xaabbccddeeff9933, 0x1122112211221122));
    field_ct c_2(&composer, uint256_t(0xaabbccddeeff9933, 0x8877665544332211, 0x1122334455667788, 0x1122112211221122));
...
TYPED_STDLIB_TEST(stdlib_field, madd)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, two_bit_table)
{
    Composer composer;
    field_ct a(witness_ct(&composer, fr::random_element()));
    field_ct b(witness_ct(&composer, fr::random_element()));
...
TYPED_STDLIB_TEST(stdlib_field, slice)
{
    Composer composer;
    // 0b11110110101001011
    //         ^      ^
...
TYPED_STDLIB_TEST(stdlib_field, slice_equal_msb_lsb)
{
    Composer composer;
    // 0b11110110101001011
    //             ^
...
TYPED_STDLIB_TEST(stdlib_field, slice_random)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, three_bit_table)
{
    Composer composer;
    field_ct a(witness_ct(&composer, fr::random_element()));
    field_ct b(witness_ct(&composer, fr::random_element()));
...
TYPED_STDLIB_TEST(stdlib_field, decompose_into_bits)
{
    ...
    auto run_success_test = [&]() {
        ...
        std::vector<barretenberg::fr> test_elements = {
            ...
            0,
            -1,
            barretenberg::fr(static_cast<uint256_t>(this->engine->get_random_uint8())),
...
TYPED_STDLIB_TEST(stdlib_field, assert_is_in_set)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, assert_is_in_set_fails)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow)
{
    Composer composer;

    barretenberg::fr base_val(this->engine->get_random_uint256());
    uint32_t exponent_val = this->engine->get_random_uint32();
...
TYPED_STDLIB_TEST(stdlib_field, pow_zero)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow_one)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow_both_constant)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow_base_constant)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow_exponent_constant)
{
    Composer composer;
...
TYPED_STDLIB_TEST(stdlib_field, pow_exponent_out_of_range)
{
    Composer composer;

    barretenberg::fr base_val(this->engine->get_random_uint256());
    uint64_t exponent_val = this->engine->get_random_uint32();
