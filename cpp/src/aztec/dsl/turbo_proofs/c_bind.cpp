...
{
    turbo_proofs::c_init_circuit_def(constraint_system_buf);
}
...
{
    return turbo_proofs::c_get_circuit_size(constraint_system_buf);
}
...
{
    return turbo_proofs::c_get_exact_circuit_size(constraint_system_buf);
}

WASM_EXPORT void standard_example__init_proving_key()
{
    auto crs_factory = std::make_unique<bonk::ReferenceStringFactory>();
    turbo_proofs::init_proving_key(std::move(crs_factory));
}
...
WASM_EXPORT void* standard_example__new_prover(uint8_t const* witness_buf)
{
    ...
    auto prover = turbo_proofs::new_prover(witness);
    return new plonk::TurboProver(std::move(prover));
}

WASM_EXPORT void standard_example__delete_prover(void* prover)
{
    delete reinterpret_cast<plonk::TurboProver*>(prover);
}
...
WASM_EXPORT bool standard_example__verify_proof(uint8_t* proof, uint32_t length)
{
    ...
    return turbo_proofs::verify_proof(pp);
}
}
...
{
    return turbo_proofs::c_init_proving_key(constraint_system_buf, pk_buf);
}
...
{
    return turbo_proofs::c_init_verification_key(pippenger, g2x, pk_buf, vk_buf);
}
...
{
    return turbo_proofs::c_new_proof(pippenger, g2x, pk_buf, constraint_system_buf, witness_buf, proof_data_buf);
}
...
{
    return turbo_proofs::c_verify_proof(g2x, vk_buf, constraint_system_buf, proof, length);
}
}
...
WASM_EXPORT uint32_t composer__smart_contract(void* pippenger,
                                              ...
                                              uint8_t** output_buf)
{
    return turbo_proofs::c_composer__smart_contract(pippenger, g2x, constraint_system_buf, output_buf);
}
                                       ...
                                       uint8_t** proof_data_buf)
{
    ...
    return turbo_proofs::c_composer__new_proof(pippenger, g2x, constraint_system_buf, witness_buf, proof_data_buf);
}
...
{
    return turbo_proofs::c_composer__verify_proof(pippenger, g2x, constraint_system_buf, proof, length);
}
}
