...
crypto::schnorr::signature convert_signature(waffle::UltraComposer& composer, std::vector<uint32_t> signature)
{
...
byte_array_ct vector_of_bytes_to_byte_array(waffle::UltraComposer& composer, std::vector<uint32_t> vector_of_bytes)
{
...
witness_ct index_to_witness(waffle::UltraComposer& composer, uint32_t index)
{
...
void create_schnorr_verify_constraints(waffle::UltraComposer& composer, const SchnorrConstraint& input)
{
    ...
    bool_ct result_bool = witness_ct(&composer, true);
    composer.assert_equal(result_bool.witness_index, input.result);
}
