...
struct MerkleMembershipConstraint {
    ...
    uint32_t root;                   // Single field element -- field_ct
    uint32_t leaf;                   // Single field element -- field_ct
    uint32_t result;                 // Single field element -- bool_t
    uint32_t index;
};

void create_merkle_check_membership_constraint(waffle::UltraComposer& composer, const MerkleMembershipConstraint& input)
{
