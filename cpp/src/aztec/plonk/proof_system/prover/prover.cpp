...
/**
 * Prints some useful information computed by the prover that can help debug a failing plonk proof.
 *  (1) If linearisation is being used, it prints a warning if the evaluation of r(X) is not 0 at X = zeta.
 *  (2) If linearisation is not being used, it prints the evaluation of t(X) at X = zeta.
 *      This value must then matched with the verifier's evaluation of t(X).
 *  (3) Some of the round challenges are printed, which must be matched with the verifier's challenges.
 *
 * In case of (1) or (2), it suggests that the prover's computation of t(X) has some error.
 * In case of (3), if the prover and verifier challenges don't match, it suggests that the prover and verifier used
 * different transcripts for generating challenges. This implies that some commitment or evaluation was replaced by a
 * wrong value during the prover's computation.
 *
 * Note that all of this would be print only if the macro DEBUG_PROVER is set to true.
 */
template <typename settings> void ProverBase<settings>::print_debug_data()
{
    if (settings::use_linearisation) {
        // Assert that r(X) at X = zeta is 0
        const auto size = key->n + 1;
        fr zeta = transcript.get_challenge_field_element("z");
        fr linear_eval = key->polynomial_cache.get("linear_poly").evaluate(zeta, size);
        if (linear_eval != fr(0)) {
            info("DEBUG_P: linear_eval ≠ 0");
        } else {
            info("DEBUG_P: linear_eval = 0");
        }
    } else {
        // Print the evaluation of the quotient polynomial at zeta, i.e. t(zeta).
        // If the quotient evaluation computed by the prover does not match the one computed by the verifier
        const auto t_eval = transcript.get_field_element("t");
        info("DEBUG_P: t_eval = ", t_eval);
    }

    // Print challenges generated by prover.
    fr alpha = transcript.get_challenge_field_element("alpha");
    fr beta = transcript.get_challenge_field_element("beta");
    fr gamma = transcript.get_challenge_field_element("beta", 1);
    fr zeta = transcript.get_challenge_field_element("z");
    info("DEBUG_P: alpha = ", alpha);
    info("DEBUG_P: beta  = ", beta);
    info("DEBUG_P: gamma = ", gamma);
    info("DEBUG_P: zeta  = ", zeta, "\n");
}
