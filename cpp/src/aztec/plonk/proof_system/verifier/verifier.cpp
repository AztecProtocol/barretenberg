...
/**
 * Prints some useful information computed by the verifier that can help debug a failing plonk proof.
 * This data is compared with the prover's corresponding data.
 */
template <typename program_settings>
void VerifierBase<program_settings>::print_debug_data(const transcript::StandardTranscript& transcript)
{
    // Print the verifier's version of t(X) evaluated at X = zeta.
    if (!program_settings::use_linearisation) {
        fr t_eval = transcript.get_field_element("t");
        info("DEBUG_V: t_eval = ", t_eval);
    }

    // Print challenges generated by verifier.
    ...
    info("DEBUG_V: alpha = ", alpha);
    info("DEBUG_V: beta  = ", beta);
    info("DEBUG_V: gamma = ", gamma);
    info("DEBUG_V: zeta  = ", zeta, "\n");
}
...
#ifdef DEBUG_PROVER
    // Prints debug data if DEBUG_PROVER is set to true.
    print_debug_data(transcript);
#endif
