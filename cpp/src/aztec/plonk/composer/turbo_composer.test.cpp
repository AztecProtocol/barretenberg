...
TEST(turbo_composer, small_scalar_multipliers)
{
    ...
    bonk::fixed_group_init_quad init_quad{ origin_points[0].x,
    ...
    for (size_t i = 0; i < num_quads; ++i) {
        bonk::fixed_group_add_quad round_quad;
    ...
    bonk::add_quad add_quad{ composer.add_variable(multiplication_transcript[num_quads].x),
...
TEST(turbo_composer, and_constraint_failure)
{
    ...
    bonk::accumulator_triple accumulators = composer.create_and_constraint(left_witness_index, right_witness_index, 2);
...
TEST(turbo_composer, big_add_gate_with_bit_extract)
{
    ...
    const auto generate_constraints = [&composer](uint32_t quad_value) {
        ...
        bonk::add_quad gate{ composer.add_variable(uint256_t(input)),
