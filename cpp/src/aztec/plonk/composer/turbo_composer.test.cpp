...
enum ScalarSize { SMALL = 0, LARGE = 1 };
enum CheckType { PROOF = 0, CIRCUIT = 1 };

template <typename Prover> class turbo_composer_tests : public testing::Test {
  public:
    static bool create_and_verify_proof(waffle::TurboComposer& composer)
    {
        bool result = false;
        if constexpr (std::is_same_v<Prover, waffle::TurboProver>) {
            auto prover = composer.create_prover();
            auto verifier = composer.create_verifier();
            auto proof = prover.construct_proof();

            result = verifier.verify_proof(proof);
        } else if constexpr (std::is_same_v<Prover, waffle::UnrolledTurboProver>) {
            ...
            info("Undefined Prover algorithm.");
        }
        return result;
    }
    ...
    static void base_case()
    {
        ...
        bool result = create_and_verify_proof(composer);
        EXPECT_EQ(result, true);
    }

    static void composer_from_serialized_keys()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        fr a = 1;
        composer.add_public_variable(a);
        ...
        auto proving_key =
            std::make_shared<waffle::proving_key>(std::move(pk_data), crs->get_prover_crs(pk_data.n + 1));
    ...
    static void add_gate_proofs()
    {
        ...
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ b_idx, a_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        composer.create_add_gate({ a_idx, b_idx, c_idx, 1, 1, -1, 0 });
        ...
        uint32_t zero_idx = composer.add_variable(0);

        composer.create_big_add_gate({ zero_idx, zero_idx, zero_idx, a_idx, 1, 1, 1, 1, -1 });

        bool result = create_and_verify_proof(composer);
        EXPECT_EQ(result, true);
    }

    static void mul_gate_proofs()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        ...
        uint32_t zero_idx = composer.add_variable(0);
        uint32_t one_idx = composer.add_variable(1);
        composer.create_big_add_gate({ zero_idx, zero_idx, zero_idx, one_idx, 1, 1, 1, 1, -1 });

        uint32_t e_idx = composer.add_variable(a - 1);
    ...
    static void scalar_multipliers(const bool scalar_size = ScalarSize::SMALL, const bool check_type = CheckType::PROOF)
    {
        constexpr size_t small_bit_size = 63;
        constexpr size_t large_bit_size = 254;
        constexpr uint64_t small_bit_mask = (1ULL << small_bit_size) - 1UL;
        const size_t num_bits = (scalar_size == ScalarSize::SMALL) ? small_bit_size : large_bit_size;
        const size_t num_quads_base = (num_bits - 1) >> 1;
        const size_t num_quads = ((num_quads_base << 1) + 1 < num_bits) ? num_quads_base + 1 : num_quads_base;
        // const size_t num_wnaf_bits = (num_quads << 1) + 1;

        const size_t initial_exponent = ((num_bits & 1) == 1) ? num_bits - 1 : num_bits;
        ...
        grumpkin::fr scalar_multiplier_base = scalar_multiplier_entropy;
        grumpkin::fr scalar_multiplier = scalar_multiplier_base.from_montgomery_form();
        if (scalar_size == ScalarSize::SMALL) {
            scalar_multiplier_base = { scalar_multiplier_entropy.data[0] & small_bit_mask, 0, 0, 0 };
            scalar_multiplier_base.data[0] = scalar_multiplier_base.data[0] & (~1ULL);
            scalar_multiplier = scalar_multiplier_base;
        }
        ...
        if ((scalar_multiplier.data[0] & 1) == 0) {
            if (scalar_size == ScalarSize::SMALL) {
                ...
                scalar_multiplier_base = scalar_multiplier_base - grumpkin::fr(2);
            }
        }
        if (scalar_size == ScalarSize::LARGE) {
            scalar_multiplier_base = scalar_multiplier_base.from_montgomery_form();
        }

        // We use an array `wnaf_entries` of size 128 for both, the small and the large scalar.
        constexpr size_t large_num_quads_base = (large_bit_size - 1) >> 1;
        constexpr size_t large_num_quads =
            ((large_num_quads_base << 1) + 1 < large_bit_size) ? large_num_quads_base + 1 : large_num_quads_base;
        uint64_t wnaf_entries[large_num_quads + 1] = { 0 };

        bool skew = false;
        if (scalar_size == ScalarSize::SMALL) {
            // The number of wnaf_bits needs to be a "constexpr" for the fixed_wnaf function.
            // That's the reason we need to call fixed_wnaf depending on the scalar size.
            constexpr size_t small_num_quads_base = (small_bit_size - 1) >> 1;
            constexpr size_t small_num_quads =
                ((small_num_quads_base << 1) + 1 < small_bit_size) ? small_num_quads_base + 1 : small_num_quads_base;

            constexpr size_t small_num_wnaf_bits = (small_num_quads << 1) + 1;
            barretenberg::wnaf::fixed_wnaf<small_num_wnaf_bits, 1, 2>(
                &scalar_multiplier_base.data[0], &wnaf_entries[0], skew, 0);
        } else {
            constexpr size_t large_num_wnaf_bits = (large_num_quads << 1) + 1;
            barretenberg::wnaf::fixed_wnaf<large_num_wnaf_bits, 1, 2>(
                &scalar_multiplier_base.data[0], &wnaf_entries[0], skew, 0);
        }

        fr accumulator_offset = fr(2).pow(static_cast<uint64_t>(initial_exponent)).invert();
        fr origin_accumulators[2]{ 1, accumulator_offset + 1 };
        ...
        bool circuit_result = composer.check_circuit();
        EXPECT_EQ(circuit_result, true);

        if (check_type == CheckType::PROOF) {
            bool result = create_and_verify_proof(composer);
            EXPECT_EQ(result, true);
        }
    ...
    static void range_constraint(const bool check_type = CheckType::PROOF)
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void range_constraint_fail()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void and_constraint(const bool check_type = CheckType::PROOF)
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void and_constraint_fail()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void xor_constraint(const bool check_type = CheckType::PROOF)
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void xor_constraint_fail()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void big_add_gate_with_bit_extract()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
    ...
    static void validate_copy_constraints()
    {
    ...
    static void check_circuit_add_gate_proofs_correct()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        fr a = 1;
        fr b = 1;
    ...
    static void check_circuit_add_gate_proofs_broken()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        fr a = 1;
        fr b = 1;
    ...
    static void check_circuit_mul_gate_proofs_correct()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        fr q[7]{ fr::random_element(), fr::random_element(), fr::random_element(), fr::random_element(),
                 fr::random_element(), fr::random_element(), fr::random_element() };
    ...
    static void check_circuit_mul_gate_proofs_broken()
    {
        waffle::TurboComposer composer = waffle::TurboComposer();
        fr q[7]{ fr::random_element(), fr::random_element(), fr::random_element(), fr::random_element(),
                 fr::random_element(), fr::random_element(), fr::random_element() };
...
typedef testing::Types<waffle::TurboProver, waffle::UnrolledTurboProver> ProverTypes;

TYPED_TEST_SUITE(turbo_composer_tests, ProverTypes);

TYPED_TEST(turbo_composer_tests, base_case)
{
    TestFixture::base_case();
};

TYPED_TEST(turbo_composer_tests, composer_from_serialized_keys)
{
    TestFixture::composer_from_serialized_keys();
};

TYPED_TEST(turbo_composer_tests, add_gate_proofs)
{
    TestFixture::add_gate_proofs();
};

TYPED_TEST(turbo_composer_tests, mul_gate_proofs)
{
    TestFixture::mul_gate_proofs();
};

TYPED_TEST(turbo_composer_tests, small_scalar_multipliers)
{
    TestFixture::scalar_multipliers(ScalarSize::SMALL);
};

TYPED_TEST(turbo_composer_tests, large_scalar_multipliers)
{
    TestFixture::scalar_multipliers(ScalarSize::LARGE);
};

TYPED_TEST(turbo_composer_tests, range_constraint)
{
    TestFixture::range_constraint();
};

TYPED_TEST(turbo_composer_tests, range_constraint_fail)
{
    TestFixture::range_constraint_fail();
};

TYPED_TEST(turbo_composer_tests, and_constraint)
{
    TestFixture::and_constraint();
};

TYPED_TEST(turbo_composer_tests, and_constraint_fail)
{
    TestFixture::and_constraint_fail();
};

TYPED_TEST(turbo_composer_tests, xor_constraint)
{
    TestFixture::xor_constraint();
};

TYPED_TEST(turbo_composer_tests, xor_constraint_fail)
{
    TestFixture::xor_constraint_fail();
};

TYPED_TEST(turbo_composer_tests, big_add_gate_with_bit_extract)
{
    TestFixture::big_add_gate_with_bit_extract();
};

TYPED_TEST(turbo_composer_tests, validate_copy_constraints)
{
    TestFixture::validate_copy_constraints();
};

// Tests that only check the circuits don't need to be run with both types of provers.
TYPED_TEST(turbo_composer_tests, check_circuit_add_gate_proofs_correct)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::check_circuit_add_gate_proofs_correct();
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_add_gate_proofs_broken)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::check_circuit_add_gate_proofs_broken();
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_mul_gate_proofs_correct)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::check_circuit_mul_gate_proofs_correct();
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_mul_gate_proofs_broken)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::check_circuit_mul_gate_proofs_broken();
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_fixed_group)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::scalar_multipliers(ScalarSize::LARGE, CheckType::CIRCUIT);
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_range_constraint)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::range_constraint(CheckType::CIRCUIT);
    }
};

TYPED_TEST(turbo_composer_tests, check_circuit_xor_constraint)
{
    if constexpr (std::is_same_v<TypeParam, waffle::TurboProver>) {
        TestFixture::xor_constraint(CheckType::CIRCUIT);
    }
};