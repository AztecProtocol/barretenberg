...
#define STANDARD_SELECTOR_REFS                                                                                         \
    auto& q_m = selectors[DummySelectors::QM];                                                                         \
    auto& q_c = selectors[DummySelectors::QC];                                                                         \
    auto& q_1 = selectors[DummySelectors::Q1];                                                                         \
    auto& q_2 = selectors[DummySelectors::Q2];                                                                         \
    auto& q_3 = selectors[DummySelectors::Q3];

/**
 * Create a dummy addition gate
 */
void DummyComposer::create_add_gate(const add_triple& in)
{
    auto result = get_variable(in.a) * in.a_scaling + get_variable(in.b) * in.b_scaling +
                  get_variable(in.c) * in.c_scaling + in.const_scaling;

    if (result != 0) {
        failure("create_add_gate");
    }
    ++num_gates;
}

/**
 * Create a dummy big addition gate
 * */
void DummyComposer::create_big_add_gate(const add_quad& in)
{
    create_add_gate(add_triple{ in.a, in.b, in.a, in.a_scaling, in.b_scaling, 0, 0 });
    create_add_gate(add_triple{ in.c, in.d, in.c, in.c_scaling, in.d_scaling, 0, 0 });
}
 ...
 * the scaling factors.
 * */
void DummyComposer::create_balanced_add_gate(const add_quad& in)
{
...
void DummyComposer::create_big_add_gate_with_bit_extraction(const add_quad& in)
{
    // blah.
    // delta = (c - 4d)
...
void DummyComposer::create_big_mul_gate(const mul_quad& in)
{
 ...
 */
void DummyComposer::create_mul_gate(const mul_triple& in)
{
    STANDARD_SELECTOR_REFS
 ...
 */
void DummyComposer::create_bool_gate(const uint32_t variable_index)
{
    STANDARD_SELECTOR_REFS
 ...
 */
void DummyComposer::create_poly_gate(const poly_triple& in)
{
    STANDARD_SELECTOR_REFS
    assert_valid_variables({ in.a, in.b, in.c });
...
void DummyComposer::create_fixed_group_add_gate_with_init(const fixed_group_add_quad& in,
...
void DummyComposer::create_fixed_group_add_gate(const fixed_group_add_quad& in)
{
    assert_valid_variables({ in.a, in.b, in.c, in.d });
...
void DummyComposer::create_fixed_group_add_gate_final(const add_quad& in)
{
...
std::vector<uint32_t> DummyComposer::decompose_into_base4_accumulators(const uint32_t witness_index,
...
accumulator_triple DummyComposer::create_logic_constraint(const uint32_t a,
                                                          const uint32_t b,
                                                          const size_t num_bits,
...
void DummyComposer::fix_witness(const uint32_t witness_index, const barretenberg::fr& witness_value)
{
    STANDARD_SELECTOR_REFS
...
uint32_t DummyComposer::put_constant_variable(const barretenberg::fr& variable)
{
...
accumulator_triple DummyComposer::create_and_constraint(const uint32_t a, const uint32_t b, const size_t num_bits)
{
    return create_logic_constraint(a, b, num_bits, false);
}

accumulator_triple DummyComposer::create_xor_constraint(const uint32_t a, const uint32_t b, const size_t num_bits)
{
 ...
 * */
std::shared_ptr<proving_key> DummyComposer::compute_proving_key()
{
 ...
 * */
std::shared_ptr<verification_key> DummyComposer::compute_verification_key()
{
 ...
 * */
void DummyComposer::compute_witness()
{
    ComposerBase::compute_witness_base<standard_settings::program_width>();
}

Verifier DummyComposer::create_verifier()
{
 ...
 * */
Prover DummyComposer::create_prover()
{
...
void DummyComposer::assert_equal_constant(uint32_t const a_idx, fr const& b, std::string const& msg)
{
 ...
 * */
bool DummyComposer::check_circuit()
{
    STANDARD_SELECTOR_REFS
