...
namespace plonk {
enum DummySelectors { QM, QC, Q1, Q2, Q3, NUM };

inline std::vector<ComposerBase::SelectorProperties> dummy_selector_properties()
{
...
class DummyComposer : public ComposerBase {
  public:
    static constexpr ComposerType type = ComposerType::DUMMY;
    static constexpr merkle::HashType merkle_hash_type = merkle::HashType::FIXED_BASE_PEDERSEN;
    static constexpr pedersen::CommitmentType commitment_type = pedersen::CommitmentType::FIXED_BASE_PEDERSEN;
    static constexpr size_t UINT_LOG2_BASE = 2;

    DummyComposer(const size_t size_hint = 0)
        : ComposerBase(DummySelectors::NUM, size_hint, dummy_selector_properties())
    {
    ...
    DummyComposer(const size_t num_selectors,
    ...
    DummyComposer(std::string const& crs_path, const size_t size_hint = 0)
        : DummyComposer(std::unique_ptr<ReferenceStringFactory>(new FileReferenceStringFactory(crs_path)), size_hint){};

    DummyComposer(std::shared_ptr<ReferenceStringFactory> const& crs_factory, const size_t size_hint = 0)
        : ComposerBase(crs_factory, DummySelectors::NUM, size_hint, dummy_selector_properties())
    {
        w_l.reserve(size_hint);
        w_r.reserve(size_hint);
        w_o.reserve(size_hint);
    ...
    DummyComposer(std::unique_ptr<ReferenceStringFactory>&& crs_factory, const size_t size_hint = 0)
        : ComposerBase(std::move(crs_factory), DummySelectors::NUM, size_hint, dummy_selector_properties())
    {
        w_l.reserve(size_hint);
        w_r.reserve(size_hint);
        w_o.reserve(size_hint);
    ...
    DummyComposer(std::shared_ptr<proving_key> const& p_key,
                  std::shared_ptr<verification_key> const& v_key,
                  size_t size_hint = 0)
        : ComposerBase(p_key, v_key, DummySelectors::NUM, size_hint, dummy_selector_properties())
    {
        w_l.reserve(size_hint);
        w_r.reserve(size_hint);
        w_o.reserve(size_hint);
    ...
    DummyComposer(DummyComposer&& other) = default;
    DummyComposer& operator=(DummyComposer&& other) = default;
    ~DummyComposer() {}
