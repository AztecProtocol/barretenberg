...
template <typename Prover> class standard_composer_tests : public testing::Test {
  public:
    static bool create_and_verify_proof(waffle::StandardComposer& composer)
    {
        bool result = false;
        if constexpr (std::is_same_v<Prover, waffle::Prover>) {
            auto prover = composer.create_prover();
            auto verifier = composer.create_verifier();
            auto proof = prover.construct_proof();

            result = verifier.verify_proof(proof);
        } else if constexpr (std::is_same_v<Prover, waffle::UnrolledProver>) {
    ...
    static void composer_from_serialized_keys()
    {
        ...
        bool result = create_and_verify_proof(composer2);
        EXPECT_EQ(result, true);
    }
    ...
    static void range_constraint()
    {
        ...
        for (size_t i = 0; i < 10; ++i) {
            ...
            std::vector<uint32_t> accumulators =
                composer.decompose_into_base4_accumulators(witness_index, 32 + extra_bits);
    ...
    static void fixed_group_add_gate_with_init()
    {
        waffle::StandardComposer composer = waffle::StandardComposer();
    ...
    static void fixed_group_add_gate()
    {
    ...
    static void check_circuit_correct()
    {
        waffle::StandardComposer composer = waffle::StandardComposer();
        fr a = fr::one();
        uint32_t a_idx = composer.add_public_variable(a);
    ...
    static void check_circuit_broken()
    {
        waffle::StandardComposer composer = waffle::StandardComposer();
        fr a = fr::one();
        uint32_t a_idx = composer.add_public_variable(a);
...
typedef testing::Types<waffle::Prover, waffle::UnrolledProver> ProverTypes;

TYPED_TEST_SUITE(standard_composer_tests, ProverTypes);

TYPED_TEST(standard_composer_tests, base_case)
{
    TestFixture::base_case();
};

TYPED_TEST(standard_composer_tests, composer_from_serialized_keys)
{
    TestFixture::composer_from_serialized_keys();
};

TYPED_TEST(standard_composer_tests, add_gate_proofs)
{
    TestFixture::add_gate_proofs();
};

TYPED_TEST(standard_composer_tests, mul_gate_proofs)
{
    TestFixture::mul_gate_proofs();
};

TYPED_TEST(standard_composer_tests, range_constraint)
{
    TestFixture::range_constraint();
};

TYPED_TEST(standard_composer_tests, range_constraint_fail)
{
    TestFixture::range_constraint_fail();
};

TYPED_TEST(standard_composer_tests, and_constraint)
{
    TestFixture::and_constraint();
};

TYPED_TEST(standard_composer_tests, xor_constraint)
{
    TestFixture::xor_constraint();
};

TYPED_TEST(standard_composer_tests, big_add_gate_with_bit_extract)
{
    TestFixture::big_add_gate_with_bit_extract();
};

TYPED_TEST(standard_composer_tests, fixed_group_add_gate_with_init)
{
    TestFixture::fixed_group_add_gate_with_init();
};

TYPED_TEST(standard_composer_tests, fixed_group_add_gate)
{
    TestFixture::fixed_group_add_gate();
};

// Tests that only check the circuits don't need to be run with both types of provers.
TYPED_TEST(standard_composer_tests, check_circuit_correct)
{
    if constexpr (std::is_same_v<TypeParam, waffle::Prover>) {
        TestFixture::check_circuit_correct();
    }
};

TYPED_TEST(standard_composer_tests, check_circuit_broken)
{
    if constexpr (std::is_same_v<TypeParam, waffle::Prover>) {
        TestFixture::check_circuit_broken();
    }
};