...
describe('pedersen', () => {
  ...
  it('pedersenPlookupCompressFields', () => {
    ...
    expect(result).to.deep.equal(
      new Fr(21568810706345846819294487214368613840251909831689369685420108292337497444070n),
    );
  });
  ...
  it('pedersenCompress', () => {
    ...
    expect(result).to.deep.equal(
      new Fr(20749503715308760529311051818180468653739005441229560405092292242074298877245n),
    );
  });

  it('pedersenPlookupCompress', () => {
    const result = api.pedersenPlookupCompress([new Fr(4n), new Fr(8n), new Fr(12n)]);
    expect(result).to.deep.equal(new Fr(4213911891650716450883144878301329379460622830501147795631256054071351353887n));
  });

  it('pedersenCompressWithHashIndex', () => {
    const result = api.pedersenCompressWithHashIndex([new Fr(4n), new Fr(8n)], 7);
    expect(result).to.deep.equal(
      new Fr(11068631634751286805527305272746775861010877976108429785597565355072506728435n),
    );
  });
  ...
  it('pedersenBufferToField', () => {
    ...
    expect(result).to.deep.equal(new Fr(4923399520610513632896240312051201308554838580477778325691012985962614653619n));
  });

  it('pedersenHashPair', () => {
    const result = api.pedersenHashPair(new Fr(4n), new Fr(8n));
    expect(result).to.deep.equal(new Fr(7508407170365331152493586290597472346478280823936748458450026785528968221772n));
  });

  it('pedersenHashMultiple', () => {
    const result = api.pedersenHashMultiple([new Fr(4n), new Fr(8n), new Fr(12n)]);
    expect(result).to.deep.equal(new Fr(641613987782189905475142047603559162464012327378197326488471789040703504911n));
  });

  it('pedersenHashMultipleWithHashIndex', () => {
    const result = api.pedersenHashMultipleWithHashIndex([new Fr(4n), new Fr(8n)], 7);
    expect(result).to.deep.equal(
      new Fr(14181105996307540196932058280391669339364159586581375348016341320932872505408n),
    );
  });
